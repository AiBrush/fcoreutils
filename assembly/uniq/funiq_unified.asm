; ============================================================
; funiq_unified.asm — GNU-compatible 'uniq' command
; Single nasm -f bin file with hand-crafted ELF header.
;
; Filter adjacent matching lines from INPUT (or stdin).
; Supports all GNU uniq flags: -c, -d, -D, -u, -i, -f, -s, -w, -z,
; --all-repeated, --group, --zero-terminated, --help, --version.
; mmap + SSE2 SIMD for file input, streaming for pipes.
;
; BUILD:
;   nasm -f bin funiq_unified.asm -o funiq && chmod +x funiq
; ============================================================
...
_start:
    ...
    mov     byte [opt_mode], MODE_NORMAL
    mov     byte [opt_flag_d], 0
    mov     byte [opt_flag_u], 0
    mov     byte [opt_count], 0
    mov     byte [opt_case_insensitive], 0
    mov     byte [opt_zero_terminated], 0
    mov     qword [opt_skip_fields], 0
    mov     qword [opt_skip_chars], 0
    mov     qword [opt_check_chars], -1     ; -1 = unlimited
    mov     byte [opt_allrep_method], ALLREP_NONE
    mov     byte [opt_group_method], GROUP_SEPARATE
    mov     qword [input_file], 0
    mov     qword [output_file], 0
    mov     qword [mmap_addr], 0
    mov     qword [mmap_len], 0
...
.short_c:
    mov     byte [opt_count], 1
    inc     rdi
    jmp     .parse_short_opts

.short_d:
    mov     byte [opt_mode], MODE_REPEATED
    mov     byte [opt_flag_d], 1
    inc     rdi
    jmp     .parse_short_opts

.short_D:
    mov     byte [opt_mode], MODE_ALL_REPEAT
    mov     byte [opt_allrep_method], ALLREP_NONE
    inc     rdi
    jmp     .parse_short_opts

.short_u:
    mov     byte [opt_mode], MODE_UNIQUE
    mov     byte [opt_flag_u], 1
    inc     rdi
    jmp     .parse_short_opts

.short_i:
    mov     byte [opt_case_insensitive], 1
    inc     rdi
    jmp     .parse_short_opts
...
.long_allrep_prepend:
    ...
    pop     rcx
    mov     byte [opt_mode], MODE_ALL_REPEAT
    mov     byte [opt_allrep_method], ALLREP_PREPEND
    jmp     .parse_next
...
.long_allrep_separate:
    ...
    pop     rcx
    mov     byte [opt_mode], MODE_ALL_REPEAT
    mov     byte [opt_allrep_method], ALLREP_SEPARATE
    jmp     .parse_next
...
.long_group_prepend:
    ...
    pop     rcx
    mov     byte [opt_mode], MODE_GROUP
    mov     byte [opt_group_method], GROUP_PREPEND
    jmp     .parse_next
.long_group_append:
    pop     rbx
    pop     rcx
    mov     byte [opt_mode], MODE_GROUP
    mov     byte [opt_group_method], GROUP_APPEND
    jmp     .parse_next
.long_group_both:
    pop     rbx
    pop     rcx
    mov     byte [opt_mode], MODE_GROUP
    mov     byte [opt_group_method], GROUP_BOTH
    jmp     .parse_next
...
.err_missing_arg_f:
    lea     rdi, [str_missing_arg_f]
    call    print_error_msg
    mov     rdi, 1
    mov     rax, SYS_EXIT
    syscall
.err_missing_arg_s:
    lea     rdi, [str_missing_arg_s]
    call    print_error_msg
    mov     rdi, 1
    mov     rax, SYS_EXIT
    syscall
.err_missing_arg_w:
    lea     rdi, [str_missing_arg_w]
    call    print_error_msg
    mov     rdi, 1
    mov     rax, SYS_EXIT
    syscall
...
.parse_done:
    ...
    cmp     byte [opt_mode], MODE_GROUP
    jne     .no_group_conflict
    cmp     byte [opt_mode], MODE_REPEATED
    jne     .check_du_unique
    cmp     byte [opt_flag_u], 1
    jne     .no_group_conflict
    jmp     .no_group_conflict
.check_du_unique:
    cmp     byte [opt_mode], MODE_UNIQUE
    jne     .no_group_conflict
    cmp     byte [opt_flag_d], 1
    jne     .no_group_conflict
...
.open_input:
    ...
    js      .input_open_error
    mov     [input_fd], eax
    mov     byte [use_mmap], 1  ; Can use mmap for file input
    jmp     .open_output

.use_stdin:
    mov     dword [input_fd], STDIN
    mov     byte [use_mmap], 0  ; Cannot mmap stdin
...
.run_uniq_mmap_ok:
    ...
    cmp     byte [opt_mode], MODE_NORMAL
    jne     .run_uniq_mmap_generic
    cmp     byte [opt_count], 0
    jne     .run_uniq_mmap_generic
    cmp     qword [opt_skip_fields], 0
    jne     .run_uniq_mmap_generic
    cmp     qword [opt_skip_chars], 0
    jne     .run_uniq_mmap_generic
    cmp     qword [opt_check_chars], -1
    jne     .run_uniq_mmap_generic
    cmp     byte [opt_case_insensitive], 0
    jne     .run_uniq_mmap_generic
    cmp     byte [opt_zero_terminated], 0
    jne     .run_uniq_mmap_generic
...
process_uniq_mmap_fast:
    ...
    mov     r13, [mmap_addr]        ; current pos
    mov     r9, r13
    add     r9, [mmap_len]          ; absolute end
...
process_uniq_mmap:
    ...
    mov     r13, [mmap_addr]    ; current pos
    mov     r14, r13
    add     r14, [mmap_len]     ; end pos
    mov     qword [count], 0
    mov     byte [first_group], 1
    mov     qword [prev_mmap_ptr], 0    ; no prev line
    mov     qword [prev_mmap_len], -1   ; -1 = no prev line
...
.pm_process_line:
    ...
    mov     rsi, rcx                ; cur_len
    mov     rdx, [prev_mmap_ptr]  ; prev_ptr
    mov     rcx, [prev_mmap_len]  ; prev_len
    ...
    mov     [prev_mmap_ptr], rdi
    mov     [prev_mmap_len], rcx
    mov     qword [count], 1
    jmp     .pm_main_loop
...
.pm_first_line:
    ...
    movzx   eax, byte [opt_group_method]
...
.pm_lines_equal:
    ...
    cmp     byte [opt_mode], MODE_ALL_REPEAT
    je      .pm_allrep_emit
    cmp     byte [opt_mode], MODE_GROUP
    je      .pm_group_emit
    jmp     .pm_main_loop
...
.pm_allrep_first_dup:
    ...
    cmp     byte [opt_allrep_method], ALLREP_PREPEND
    je      .pm_allrep_prepend_sep
    cmp     byte [opt_allrep_method], ALLREP_SEPARATE
...
.pm_allrep_emit_both:
    ...
    push    rdi
    push    rcx
    mov     rdi, [prev_mmap_ptr]
    mov     rsi, [prev_mmap_len]
    call    emit_line_direct
    pop     rcx
    pop     rdi
    ; Emit current line
...
.eld_direct_write:
    ...
    ; Write delimiter
    movzx   eax, byte [delimiter]
    mov     [count_buf], al     ; reuse count_buf temporarily
    mov     edi, [output_fd]
    lea     rsi, [count_buf]
    mov     edx, 1
    call    asm_write_all
...
process_uniq_slow:
    ...
    ; Initialize state
    mov     qword [prev_line_len], -1       ; -1 = no previous line
    mov     qword [cur_line_len], 0
    mov     qword [count], 0
    mov     byte [first_group], 1
    mov     qword [read_buf_pos], 0
    mov     qword [read_buf_end], 0
...
.pus_allrep_emit:
    ...
    lea     rdi, [cur_line_buf]
    mov     rsi, [cur_line_len]
    call    emit_line_direct
    jmp     .pus_main_loop
...
compare_lines_slow:
    ...
    lea     r8, [prev_line_buf]
    mov     r9, [prev_line_len]
    lea     r10, [cur_line_buf]
    mov     r11, [cur_line_len]
...
read_line:
    ...
    mov     qword [cur_line_len], 0
    movzx   r15d, byte [delimiter]

.rl_scan:
    mov     rax, [read_buf_pos]
    cmp     rax, [read_buf_end]
    jl      .rl_have_data
    ...
    mov     qword [read_buf_pos], 0
    mov     [read_buf_end], rax

.rl_have_data:
    mov     r14, [read_buf_pos]
    mov     rbx, [read_buf_end]
...
.rl_simd_found:
    ...
    pop     rcx
    pop     rcx
    add     [cur_line_len], rcx

.rl_found_at_pos:
    add     r14, rcx
    inc     r14
    mov     [read_buf_pos], r14
    xor     eax, eax
    pop     r15
    pop     r14
    pop     rbx
...
err_extra_operand:
    ...
    mov     rdi, STDERR
    lea     rsi, [str_extra_operand]
    mov     rdx, str_extra_operand_len
    call    asm_write_all

    mov     rdi, STDERR
    lea     rsi, [str_quote_open]
    mov     rdx, 1
    call    asm_write_all
