...
BSS_SIZE = 65536
TOOL_NAME = "uniq"
BIN = str(Path(__file__).resolve().parent.parent / "funiq")
GNU = "/usr/bin/uniq"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "funiq", "proc: /proc/PID/exe points to funiq")
        except Exception as e:
...
def test_concurrency():
    ...
    # sort | uniq pipe chain
    data = b"cherry\napple\nbanana\napple\ncherry\n"
    script = f'printf "cherry\\napple\\nbanana\\napple\\ncherry\\n" | sort | {BIN}'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT)
    expected = b"apple\nbanana\ncherry\n"
    report_result(p.stdout == expected, "concurrency: sort | uniq pipe chain")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: UNIQ TESTS
# =============================================================================

def test_uniq_specific():
    log("\n=== Uniq-Specific Tests ===")
    ...
    report_result(out_a == out_g, "uniq: basic dedup matches GNU")
    report_result(out_a == b"aaa\nbbb\nccc\n", "uniq: basic dedup correct output")

    # No duplicates
    data = b"aaa\nbbb\nccc\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: no duplicates matches GNU")

    # All duplicates
    data = b"aaa\naaa\naaa\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: all duplicates matches GNU")

    # -c (count)
    data = b"aaa\naaa\nbbb\nccc\nccc\nccc\n"
    rc_a, out_a, _ = run_asm(["-c"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-c"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -c (count) matches GNU")

    # -d (duplicates only)
    data = b"aaa\naaa\nbbb\nccc\nccc\n"
    rc_a, out_a, _ = run_asm(["-d"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-d"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -d (duplicates only) matches GNU")

    # -u (unique only)
    data = b"aaa\naaa\nbbb\nccc\nccc\n"
    rc_a, out_a, _ = run_asm(["-u"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-u"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -u (unique only) matches GNU")

    # -i (case insensitive)
    data = b"Hello\nhello\nHELLO\nWorld\n"
    rc_a, out_a, _ = run_asm(["-i"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-i"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -i (case insensitive) matches GNU")

    # -f N (skip fields)
    data = b"field1 aaa\nfield2 aaa\nfield1 bbb\n"
    rc_a, out_a, _ = run_asm(["-f", "1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-f", "1"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -f 1 (skip fields) matches GNU")

    # -s N (skip chars)
    data = b"XXXaaa\nYYYaaa\nZZZbbb\n"
    rc_a, out_a, _ = run_asm(["-s", "3"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", "3"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -s 3 (skip chars) matches GNU")

    # -w N (check chars)
    data = b"aaaxxx\naaayyy\nbbbxxx\n"
    rc_a, out_a, _ = run_asm(["-w", "3"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w", "3"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -w 3 (check chars) matches GNU")

    # -c -d combined
    data = b"aaa\naaa\nbbb\nccc\nccc\n"
    rc_a, out_a, _ = run_asm(["-c", "-d"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-c", "-d"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -c -d combined matches GNU")

    # -c -u combined
    data = b"aaa\naaa\nbbb\nccc\nccc\n"
    rc_a, out_a, _ = run_asm(["-c", "-u"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-c", "-u"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -c -u combined matches GNU")

    # Empty input
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "uniq: empty input matches GNU")

    # Single line
    data = b"only\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: single line matches GNU")

    # Single newline
    rc_a, out_a, _ = run_asm([], stdin_data=b"\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"\n")
    report_result(out_a == out_g, "uniq: single newline matches GNU")

    # No trailing newline
    data = b"hello\nhello"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: no trailing newline matches GNU")
    ...
    try:
        ...
        report_result(out_a == out_g, "uniq: input file argument matches GNU")
    finally:
        os.unlink(tmpfile)
    ...
    try:
        run_asm([infile, outfile_a])
        run_gnu([infile, outfile_g])
        if os.path.exists(outfile_a) and os.path.exists(outfile_g):
            content_a = open(outfile_a, 'rb').read()
            content_g = open(outfile_g, 'rb').read()
            report_result(content_a == content_g, "uniq: output file argument matches GNU")
        else:
            report_result(False, "uniq: output file argument matches GNU")
    finally:
    ...
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: many duplicate runs matches GNU")
    ...
    report_result(out_a == out_g, "uniq: large input (10K lines) matches GNU")

    # Spaces and tabs
    data = b"  hello  \n  hello  \n\tworld\t\n\tworld\t\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: spaces and tabs matches GNU")

    # Empty lines as duplicates
    data = b"\n\n\nhello\n\n\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "uniq: empty lines as duplicates matches GNU")

    # --help
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "uniq: --help works")

    # --version
    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "uniq: --version works")

    # -f and -s combined
    data = b"X field aaa\nY field aaa\nZ field bbb\n"
    rc_a, out_a, _ = run_asm(["-f", "1", "-s", "1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-f", "1", "-s", "1"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -f 1 -s 1 combined matches GNU")

    # -i -c combined
    data = b"Hello\nhello\nWorld\nworld\nWORLD\n"
    rc_a, out_a, _ = run_asm(["-i", "-c"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-i", "-c"], stdin_data=data)
    report_result(out_a == out_g, "uniq: -i -c combined matches GNU")
    ...
    report_result(out_a == out_g, "uniq: binary data lines matches GNU")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (funiq) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
