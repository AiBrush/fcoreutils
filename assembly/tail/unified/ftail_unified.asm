...
help_text:
    ...
    db "                             or use -n +NUM to skip NUM-1 lines at the start", 10
    ...
    db "                             to see if it has been unlinked or renamed", 10
    db "                             (this is the usual case of rotated log files);", 10
    db "                             with inotify, this option is rarely useful", 10
    db "      --pid=PID            with -f, terminate after process ID, PID dies;", 10
    db "                             can be repeated to watch multiple processes", 10
    ...
    db "                             with inotify and --pid=P, check process P at", 10
    db "                             least once every N seconds", 10
    ...
    db 10
    db "With --follow (-f), tail defaults to following the file descriptor, which", 10
    db "means that even if a tail'ed file is renamed, tail will continue to track", 10
    db "its end.  This default behavior is not desirable when you really want to", 10
    db "track the actual name of the file, not the file descriptor (e.g., log", 10
    db "rotation).  Use --follow=name in that case.  That causes tail to track the", 10
    db "named file in a way that accommodates renaming, removal and creation.", 10
    db 10
    db "GNU coreutils online help: <https://www.gnu.org/software/coreutils/>", 10
    db "Full documentation <https://www.gnu.org/software/coreutils/tail>", 10
    db "or available locally via: info '(coreutils) tail invocation'", 10
help_text_len equ $ - help_text

version_text:
    db "tail (GNU coreutils) 9.7", 10
    ...
    db 10
    db "Written by Paul Rubin, David MacKenzie, Ian Lance Taylor,", 10
...
err_try_suffix:   db 0xE2, 0x80, 0x99, 10
