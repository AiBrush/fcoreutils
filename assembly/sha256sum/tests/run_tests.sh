#!/bin/bash
# Test suite for fsha256sum
# Usage: bash tests/run_tests.sh ./fsha256sum
...
# ── Setup temp files ─────────────────────────────────────────
TMPDIR=$(mktemp -d /tmp/sha256_test.XXXXXX)
...
$GNU "$TMPDIR/hello.txt" > "$TMPDIR/hello.sha256"
$GNU "$TMPDIR/hello.txt" "$TMPDIR/world.txt" > "$TMPDIR/multi.sha256"
...
# ── -c (check mode) ─────────────────────────────────────────
run_test "-c check single file" -c "$TMPDIR/hello.sha256"
run_test "-c check multiple files" -c "$TMPDIR/multi.sha256"
