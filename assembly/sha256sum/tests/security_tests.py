#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fsha256sum (assembly sha256sum).

fsha256sum is a GNU-compatible 'sha256sum' written in x86-64 Linux assembly.
It computes SHA-256 message digests.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (sha256sum: SHA-256 checksum behavior)
"""
...
BSS_SIZE = 65536
TOOL_NAME = "sha256sum"
BIN = str(Path(__file__).resolve().parent.parent / "fsha256sum")
GNU = "/usr/bin/sha256sum"
...
def sha256(data):
    """Compute SHA-256 hash of data using Python hashlib."""
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fsha256sum", "proc: /proc/PID/exe points to fsha256sum")
        except Exception as e:
...
# =============================================================================
#                     13. TOOL-SPECIFIC: SHA256SUM TESTS
# =============================================================================

def test_sha256sum_specific():
    log("\n=== Sha256sum-Specific Tests ===")

    # Known hash test vectors (FIPS 180-4)
    vectors = [
    ...
    for data, expected_hash in vectors:
        ...
        if rc == 0:
            ...
            report_result(False, f"sha256: vector failed (rc={rc})")
    ...
    for data, expected_hash in vectors:
        ...
        if rc_a == 0 and rc_g == 0:
            ...
            report_result(hash_a == hash_g, f"sha256: GNU match '{data[:20].decode(errors='replace')}...'")
    ...
    if rc == 0:
        ...
        parts = out_str.split()
        report_result(len(parts) >= 2 and len(parts[0]) == 64 and parts[1] == "-",
                     "sha256: output format 'hash  -'")
    else:
        report_result(False, "sha256: output format check (command failed)")

    # Compare with GNU output format
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "sha256: output format matches GNU exactly")

    # Binary data hashing (all 256 byte values)
    all_bytes = bytes(range(256))
    expected = sha256(all_bytes)
    rc, out, _ = run_asm([], stdin_data=all_bytes)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha256: all 256 byte values hash correct")

    # Large file hashing (1MB+)
    large_data = os.urandom(1024 * 1024)
    expected = sha256(large_data)
    rc, out, _ = run_asm([], stdin_data=large_data, timeout=15)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha256: 1MB hash correct")
    else:
        report_result(False, "sha256: 1MB hash (command failed)")
    ...
    for i in range(20):
        ...
        if rc == 0:
            output_hash = out.decode().strip().split()[0] if out else ""
            report_result(output_hash == expected, f"sha256: random {size} bytes hash correct")
        else:
            report_result(False, f"sha256: random {size} bytes (command failed)")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(fpath, "wb") as f:
            f.write(b"hello\n")
        rc_a, out_a, _ = run_asm([fpath])
        rc_g, out_g, _ = run_gnu([fpath])
        report_result(out_a == out_g and rc_a == rc_g, "sha256: file hash matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha256sum: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha256sum: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fsha256sum) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
