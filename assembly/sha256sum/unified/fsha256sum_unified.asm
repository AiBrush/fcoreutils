; fsha256sum_unified.asm
; Hand-crafted minimal ELF binary for fsha256sum
; Fully self-contained — no libc, no linker needed
; Build: nasm -f bin fsha256sum_unified.asm -o fsha256sum && chmod +x fsha256sum
...
; ── SHA-256 Init ──
sha256_init:
    mov dword [hash_state],    0x6a09e667
    mov dword [hash_state+4],  0xbb67ae85
    mov dword [hash_state+8],  0x3c6ef372
    mov dword [hash_state+12], 0xa54ff53a
    mov dword [hash_state+16], 0x510e527f
    mov dword [hash_state+20], 0x9b05688c
    mov dword [hash_state+24], 0x1f83d9ab
    mov dword [hash_state+28], 0x5be0cd19
    mov qword [msg_len], 0
...
sha256_transform:
    ...
    ; We'll use: eax=a, ebx=b, ecx=c, edx=d, r8d=e, r9d=f, r10d=g, r11d=h
    ; Save initial hash for adding at end
    mov  eax, [hash_state]
    mov  [rsp],    eax
...
; ── SHA-256 to hex (big-endian output, 64 hex chars) ──
sha256_to_hex:
    xor ecx, ecx
.loop:
    cmp ecx, 8            ; 8 dwords = 32 bytes
    jge .done
    mov eax, [hash_state + rcx*4]
    ; Hash state words are in native format; output high nibble first (big-endian interpretation)
    ; Convert 4 bytes to 8 hex chars
    mov edx, ecx
...
.output_tag:
    mov rdi, out_buf
    mov rsi, str_sha256_tag
    mov ecx, str_sha256_tag_len
    rep movsb
    mov rsi, r12
.tcf: lodsb
    test al, al
...
.have_line:
    ...
    inc rsi
.no_esc:
    ; Try BSD-style "SHA256 (" tag format
    cmp dword [rsi], 'SHA2'
    jne .try_std
    cmp word [rsi+4], '56'
    jne .try_std
...
; @@DATA_START@@
str_help:
    db "Usage: sha256sum [OPTION]... [FILE]...", 10
    db "Print or check SHA256 (256-bit) checksums.", 10
    db 10
    ...
    db "Full documentation <https://www.gnu.org/software/coreutils/sha256sum>", 10
    db "or available locally via: info '(coreutils) sha256sum invocation'", 10
str_help_len equ $ - str_help

str_version:
    db "sha256sum (GNU coreutils) 9.7", 10
...
str_sha256_tag: db "SHA256 ("
str_sha256_tag_len equ $ - str_sha256_tag
str_tag_eq: db ") = "
...
err_unrec: db "sha256sum: unrecognized option '"
err_unrec_len equ $ - err_unrec
err_inval: db "sha256sum: invalid option -- '"
err_inval_len equ $ - err_inval
err_suffix:   db 0xE2, 0x80, 0x99, 10
err_suffix_len equ $ - err_suffix

err_tag_check: db "sha256sum: the --tag option is meaningless when verifying checksums", 10
               db "Try 'sha256sum --help' for more information.", 10
err_tag_check_len equ $ - err_tag_check

str_warn_prefix: db "sha256sum: WARNING: "
...
str_no_proper: db ": no properly formatted SHA256 checksum lines found", 10
...
str_improperly: db ": improperly formatted SHA256 checksum line", 10
