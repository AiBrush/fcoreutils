; ============================================================
; fwc_unified.asm — GNU-compatible 'wc' command
...
; SIMD: SSE2 for newline counting and whitespace classification.
; UTF-8 locale detection for -m char counting.
;
; BUILD:
;   nasm -f bin fwc_unified.asm -o fwc && chmod +x fwc
; ============================================================
...
; --- String routines (inlined from lib/str.asm) ---
...
; ===============================================================
; DATA (constants — placed before code in flat binary)
; ===============================================================
...
str_help:
    ...
    db "more than one FILE is specified.  A word is a nonempty sequence of non white", 10
    db "space delimited by white space characters or by start or end of input.", 10
    db 10
...
_start:
    ...
    xor     eax, eax
    mov     [file_count], rax
    mov     byte [show_flags], 0
    mov     byte [total_mode], TOTAL_AUTO
    mov     qword [files0_from_ptr], 0
    mov     qword [outbuf_pos], 0
    mov     byte [stdin_implicit], 0
    mov     byte [has_stdin], 0
    mov     byte [has_nonreg], 0
    mov     qword [result_count], 0
    ...
    cmp     byte [show_flags], 0
    jne     .flags_set
    mov     byte [show_flags], FLAG_LINES | FLAG_WORDS | FLAG_BYTES
.flags_set:
    ...
    lea     rax, [str_dash]
    mov     [file_ptrs], rax
    mov     qword [file_count], 1
    mov     byte [stdin_implicit], 1
.has_files:
...
.set_lines:
    or      byte [show_flags], FLAG_LINES
    inc     rdi
    jmp     .short_loop
.set_words:
    or      byte [show_flags], FLAG_WORDS
    inc     rdi
    jmp     .short_loop
.set_bytes:
    or      byte [show_flags], FLAG_BYTES
    inc     rdi
    jmp     .short_loop
.set_chars:
    or      byte [show_flags], FLAG_CHARS
    inc     rdi
    jmp     .short_loop
.set_maxlen:
    or      byte [show_flags], FLAG_MAXLEN
    inc     rdi
    jmp     .short_loop
...
.do_total:
    ...
    lea     rdi, [str_inv_total_pre]
    call    asm_strlen
    mov     rdx, rax
    lea     rsi, [str_inv_total_pre]
    mov     rdi, STDERR
    call    asm_write_all
    ...
    mov     rdi, STDERR
    lea     rsi, [str_inv_total_mid]
    mov     rdx, str_inv_total_mid_len
    call    asm_write_all

    mov     rdi, STDERR
    lea     rsi, [str_inv_total_valid]
...
.total_auto:
    ...
    jmp     .next_arg
.total_always:
    mov     byte [total_mode], TOTAL_ALWAYS
    jmp     .next_arg
.total_never:
    mov     byte [total_mode], TOTAL_NEVER
    jmp     .next_arg
.total_only:
    mov     byte [total_mode], TOTAL_ONLY
    jmp     .next_arg

.add_file:
    ; Check if --files0-from is active (can't combine with file operands)
    cmp     qword [files0_from_ptr], 0
    jne     .files0_conflict
...
.files0_conflict:
    ...
    mov     rdi, STDERR
    lea     rsi, [str_extra_operand2]
    mov     rdx, 2
    call    asm_write_all

    ; "file operands cannot be combined with --files0-from"
    mov     rdi, STDERR
    lea     rsi, [str_files0_combined]
...
.do_count:
    ...
    mov     byte [prev_was_space], 1   ; start "in whitespace" for word counting
...
.accumulate:
    ...
    add     [total_chars], rax
    mov     rax, [cur_maxlen]
    cmp     rax, [total_maxlen]
    jbe     .no_update_maxlen
    mov     [total_maxlen], rax
.no_update_maxlen:
    ...
    jne     .store_name
    ; It's "-" (stdin)
    cmp     byte [stdin_implicit], 1
    jne     .store_name             ; explicit "-", keep it
    lea     rax, [empty_str]    ; implicit stdin: empty display name
.store_name:
    mov     [rdi + rbp * 8], rax
...
.full_count:
    ...
    mov     r11, [cur_maxlen]   ; max line length
    mov     rbp, [cur_line_len] ; current line length (persisted across chunks)
    movzx   ebx, byte [prev_was_space]  ; prev whitespace state
...
.full_count_done:
    ...
    add     [cur_chars], r10
    mov     [cur_maxlen], r11
    mov     [cur_line_len], rbp     ; persist current line length across chunks
    mov     [prev_was_space], bl
...
compute_width_and_print:
    ...
    mov     r12, [result_count] ; number of successful results
...
.print_total:
    ...
    ; Print total line
    push    rbx
    lea     rdi, [total_lines]  ; total results
    movzx   eax, byte [total_mode]
    cmp     al, TOTAL_ONLY
    je      .total_no_label
    lea     rsi, [str_total]    ; "total"
    jmp     .print_total_line
.total_no_label:
    lea     rsi, [empty_str]    ; no label for --total=only
.print_total_line:
    pop     rbx
    call    print_one_result
...
read_buf        equ bss_base + 0                    ; BUF_SIZE = 65536
outbuf          equ read_buf + BUF_SIZE              ; OUTBUF_SIZE = 8192
outbuf_pos      equ outbuf + OUTBUF_SIZE             ; 8
stat_buf        equ outbuf_pos + 8                   ; STAT_STRUCT_SIZE = 144
file_ptrs       equ stat_buf + STAT_STRUCT_SIZE      ; MAX_FILES * 8 = 32768
file_count      equ file_ptrs + MAX_FILES * 8        ; 8
results         equ file_count + 8                   ; (MAX_FILES+1) * 5 * 8 = 163880
result_names    equ results + (MAX_FILES + 1) * 5 * 8 ; MAX_FILES * 8 = 32768
result_count    equ result_names + MAX_FILES * 8     ; 8
files0_buf      equ result_count + 8                 ; BUF_SIZE = 65536
itoa_buf        equ files0_buf + BUF_SIZE            ; ITOA_BUF_SIZE = 24
line_buf        equ itoa_buf + ITOA_BUF_SIZE         ; 4352
show_flags      equ line_buf + 4352                  ; 1
total_mode      equ show_flags + 1                   ; 1
had_error       equ total_mode + 1                   ; 1
prev_was_space  equ had_error + 1                    ; 1
cur_flags       equ prev_was_space + 1               ; 1
stdin_implicit  equ cur_flags + 1                    ; 1
has_stdin       equ stdin_implicit + 1               ; 1
has_nonreg      equ has_stdin + 1                    ; 1
utf8_mode       equ has_nonreg + 1                   ; 1
files0_from_ptr equ utf8_mode + 8                    ; 8 (next aligned)
cur_lines       equ files0_from_ptr + 8              ; 8
cur_words       equ cur_lines + 8                    ; 8
cur_bytes       equ cur_words + 8                    ; 8
cur_chars       equ cur_bytes + 8                    ; 8
cur_maxlen      equ cur_chars + 8                    ; 8
cur_line_len    equ cur_maxlen + 8                   ; 8
total_lines     equ cur_line_len + 8                 ; 8
total_words     equ total_lines + 8                  ; 8
total_bytes     equ total_words + 8                  ; 8
total_chars     equ total_bytes + 8                  ; 8
total_maxlen    equ total_chars + 8                  ; 8

bss_end         equ total_maxlen + 8
