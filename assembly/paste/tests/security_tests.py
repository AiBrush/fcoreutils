...
TIMEOUT = 5
BSS_SIZE = 262144  # OUT_BUF_SIZE from the assembly
TOOL_NAME = "paste"
BIN = str(Path(__file__).resolve().parent.parent / "fpaste")
GNU = "/usr/bin/paste"
...
def test_syscall_surface():
    ...
    try:
        rc, out, err = run(["strace", "-f", "-e", "trace=%network", BIN, test_file])
        ...
        rc, out, err = run(["strace", "-f", "-e", "trace=brk,mmap,mprotect", BIN, test_file])
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(True, "proc: RWX check (ELF binary)")
        except Exception as e:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fpaste", "proc: /proc/PID/exe points to fpaste")
        except Exception as e:
...
def test_memory_safety():
    ...
    for desc, data in [
        ...
        ("CRLF line endings", b"a\r\nb\r\n"),
        ("embedded nulls", b"a\x00b\nc\x00d\n"),
...
def test_signal_safety():
    ...
    try:
        script = f'{BIN} {test_file} | head -1 >/dev/null 2>/dev/null; echo $?'
        p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    ...
    try:
        ...
        for _ in range(trials):
            rc = os.system(f'{BIN} {test_file} | head -c 1 >/dev/null 2>/dev/null')
            if rc == 0: ok_count += 1
...
def test_input_fuzzing():
    ...
    pathological = [
        ("64KB nulls", b"\x00" * 65536),
        ("64KB newlines", b"\n" * 65536),
        ("64KB 0xFF", b"\xff" * 65536),
...
def test_output_integrity():
    ...
    rc, out, err = run_asm(["-"], stdin_data=b"a\nb\n")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: PASTE TESTS
# =============================================================================

def test_paste_specific():
    log("\n=== Paste-Specific Tests ===")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Basic paste
        rc_a, out_a, _ = run_asm([fa, fb])
        rc_g, out_g, _ = run_gnu([fa, fb])
        report_result(out_a == out_g, "paste: two files matches GNU")

        # Three files
        rc_a, out_a, _ = run_asm([fa, fb, fc])
        rc_g, out_g, _ = run_gnu([fa, fb, fc])
        report_result(out_a == out_g, "paste: three files matches GNU")

        # Custom delimiter
        rc_a, out_a, _ = run_asm(["-d", ":", fa, fb])
        rc_g, out_g, _ = run_gnu(["-d", ":", fa, fb])
        report_result(out_a == out_g, "paste: -d : matches GNU")

        # Multi-char delimiter
        rc_a, out_a, _ = run_asm(["-d", ":,", fa, fb, fc])
        rc_g, out_g, _ = run_gnu(["-d", ":,", fa, fb, fc])
        report_result(out_a == out_g, "paste: -d :, matches GNU")

        # Empty delimiter
        rc_a, out_a, _ = run_asm(["-d", "", fa, fb])
        rc_g, out_g, _ = run_gnu(["-d", "", fa, fb])
        report_result(out_a == out_g, "paste: empty delimiter matches GNU")

        # Serial mode
        rc_a, out_a, _ = run_asm(["-s", fa])
        rc_g, out_g, _ = run_gnu(["-s", fa])
        report_result(out_a == out_g, "paste: -s matches GNU")

        # Serial multiple
        rc_a, out_a, _ = run_asm(["-s", fa, fb])
        rc_g, out_g, _ = run_gnu(["-s", fa, fb])
        report_result(out_a == out_g, "paste: -s multiple matches GNU")

        # Serial empty
        rc_a, out_a, _ = run_asm(["-s", fempty])
        rc_g, out_g, _ = run_gnu(["-s", fempty])
        report_result(out_a == out_g, "paste: -s empty matches GNU")

        # Empty file
        rc_a, out_a, _ = run_asm([fempty])
        rc_g, out_g, _ = run_gnu([fempty])
        report_result(out_a == out_g, "paste: empty file matches GNU")

        # Unequal files
        fshort = os.path.join(tmpdir, "short.txt")
        with open(fshort, "w") as f: f.write("a\n")
        rc_a, out_a, _ = run_asm([fa, fshort])
        rc_g, out_g, _ = run_gnu([fa, fshort])
        report_result(out_a == out_g, "paste: unequal files matches GNU")

        # Stdin
        rc_a, out_a, _ = run_asm(["-"], stdin_data=b"hello\nworld\n")
        rc_g, out_g, _ = run_gnu(["-"], stdin_data=b"hello\nworld\n")
        report_result(out_a == out_g, "paste: stdin matches GNU")

        # Stdin paired
        rc_a, out_a, _ = run_asm(["-", "-"], stdin_data=b"1\n2\n3\n4\n")
        rc_g, out_g, _ = run_gnu(["-", "-"], stdin_data=b"1\n2\n3\n4\n")
        report_result(out_a == out_g, "paste: stdin paired matches GNU")

        # Nonexistent file
        rc_a, _, _ = run_asm(["/nonexistent_xyz_paste"])
        rc_g, _, _ = run_gnu(["/nonexistent_xyz_paste"])
        report_result(rc_a == rc_g, "paste: nonexistent file exit code matches GNU")
        ...
        rc_a, out_a, _ = run_asm([flarge, flarge], timeout=10)
        rc_g, out_g, _ = run_gnu([flarge, flarge], timeout=10)
        report_result(out_a == out_g, "paste: large input matches GNU")

        # --help/--version
        rc_a, out_a, _ = run_asm(["--help"])
        report_result(rc_a == 0 and len(out_a) > 0, "paste: --help works")

        rc_a, out_a, _ = run_asm(["--version"])
        report_result(rc_a == 0 and len(out_a) > 0, "paste: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fpaste) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
