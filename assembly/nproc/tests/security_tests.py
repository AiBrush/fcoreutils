#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fnproc.

fnproc is a GNU-compatible 'nproc' written in x86-64 Linux assembly.
It prints the number of available processing units.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (nproc: CPU count behavior)
"""
...
def find_binary():
    ...
    for name in ["fnproc_release", "fnproc"]:
...
def check_syscall_surface():
    ...
    cmd = ["strace", "-f", "-e", "trace=%process,%network,write,read,openat,open,creat,brk,mmap,mprotect,sched_getaffinity",
           BIN]
    rc, out, err = run(cmd)
    ...
    # nproc should call sched_getaffinity
    sched_calls = [l for l in lines if "sched_getaffinity(" in l]
    report_result(len(sched_calls) >= 1, "syscall: sched_getaffinity called (expected)")
...
def check_memory_safety():
    ...
    rc, out, err = run([BIN] + [f"--ignore={i}" for i in range(100)][:1] + ["x" * 10] * 999)
    report_result(rc >= 0 and rc < 128, "memory: no crash with many args")
...
def check_output_integrity():
    ...
    num_str = out.strip()
    report_result(num_str.isdigit() and int(num_str) >= 1, "output: valid CPU count >= 1")
    ...
    if gnu_path:
        ...
        report_result(out_f == out_g and rc_f == rc_g, "output: matches GNU nproc")

        rc_f, out_f, _ = run([BIN, "--all"])
        rc_g, out_g, _ = run([gnu_path, "--all"])
        report_result(out_f == out_g and rc_f == rc_g, "output: --all matches GNU")
...
def check_tool_specific():
    ...
    report_result(rc == 0 and cpu_count >= 1, f"nproc: outputs valid CPU count ({cpu_count})")

    # --all flag
    rc, out, err = run([BIN, "--all"])
    all_count = int(out.strip()) if out.strip().isdigit() else 0
    report_result(rc == 0 and all_count >= 1, f"nproc: --all outputs valid count ({all_count})")
    report_result(all_count >= cpu_count, "nproc: --all >= default count")

    # --ignore=N
    rc, out, err = run([BIN, "--ignore=1"])
    ign1 = int(out.strip()) if out.strip().isdigit() else 0
    report_result(rc == 0 and ign1 >= 1, f"nproc: --ignore=1 outputs {ign1}")

    # --ignore with large N should clamp to 1
    rc, out, err = run([BIN, "--ignore=99999"])
    report_result(out.strip() == b"1", "nproc: --ignore=99999 clamps to 1")

    # --ignore=0 same as no flag
    rc, out, err = run([BIN, "--ignore=0"])
    ign0 = int(out.strip()) if out.strip().isdigit() else 0
    report_result(ign0 == cpu_count, "nproc: --ignore=0 same as default")

    # Output format: just a number and newline
    rc, out, err = run([BIN])
    report_result(out == str(cpu_count).encode() + b"\n", "nproc: output format is NUMBER\\n")

    # No extra output on stderr
    rc, out, err = run([BIN])
    report_result(err == b"", "nproc: no stderr output on success")
    ...
    if gnu_path:
        for args in [[], ["--all"], ["--ignore=0"], ["--ignore=1"], ["--ignore=2"]]:
            ...
            report_result(out_f == out_g, f"nproc: matches GNU for {args or '(no args)'}")
