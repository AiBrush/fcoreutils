...
BSS_SIZE = 65536
TOOL_NAME = "od"
BIN = str(Path(__file__).resolve().parent.parent / "fod")
GNU = "/usr/bin/od"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fod", "proc: /proc/PID/exe points to fod")
        except Exception as e:
...
def test_memory_safety():
    ...
    report_result(rc < 128, "mem: single chunk >BSS_SIZE no crash")

    tiny_lines = b"\n" * 1000000
    rc, _, _ = run_asm([], stdin_data=tiny_lines)
    report_result(rc < 128, "mem: 1M newlines no crash")

    log("\n--- Boundary Value Analysis ---")
    for desc, data in [
        ("no trailing newline", b"hello"),
        ("only newlines", b"\n" * 50),
        ("1MB binary", os.urandom(1024 * 1024)),
        ("CRLF data", b"line1\r\nline2\r\nline3\r\n"),
...
def test_concurrency():
    ...
    # od pipe chain: od output is deterministic
    data = b"AB"
    rc1, out1, _ = run_asm([], stdin_data=data)
    rc2, out2, _ = run_asm([], stdin_data=data)
    report_result(out1 == out2, "concurrency: pipe deterministic output")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: OD TESTS
# =============================================================================

def test_od_specific():
    log("\n=== Od-Specific Tests ===")
    ...
    report_result(out_a == out_g, "od: default octal dump matches GNU")

    # -A d (decimal addresses)
    data = b"hello world\n"
    rc_a, out_a, _ = run_asm(["-A", "d"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-A", "d"], stdin_data=data)
    report_result(out_a == out_g, "od: -A d (decimal addresses) matches GNU")

    # -A o (octal addresses, default)
    rc_a, out_a, _ = run_asm(["-A", "o"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-A", "o"], stdin_data=data)
    report_result(out_a == out_g, "od: -A o (octal addresses) matches GNU")

    # -A x (hex addresses)
    rc_a, out_a, _ = run_asm(["-A", "x"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-A", "x"], stdin_data=data)
    report_result(out_a == out_g, "od: -A x (hex addresses) matches GNU")

    # -A n (no addresses)
    rc_a, out_a, _ = run_asm(["-A", "n"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-A", "n"], stdin_data=data)
    report_result(out_a == out_g, "od: -A n (no addresses) matches GNU")

    # -t x1 (hex bytes)
    data = b"\x00\x01\x02\x0a\xff"
    rc_a, out_a, _ = run_asm(["-t", "x1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x1"], stdin_data=data)
    report_result(out_a == out_g, "od: -t x1 (hex bytes) matches GNU")

    # -t x2 (hex 2-byte)
    data = b"\x00\x01\x02\x03\x04\x05"
    rc_a, out_a, _ = run_asm(["-t", "x2"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x2"], stdin_data=data)
    report_result(out_a == out_g, "od: -t x2 (hex 2-byte) matches GNU")

    # -t o1 (octal bytes)
    data = b"hello"
    rc_a, out_a, _ = run_asm(["-t", "o1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "o1"], stdin_data=data)
    report_result(out_a == out_g, "od: -t o1 (octal bytes) matches GNU")

    # -t d1 (decimal bytes)
    data = b"AB\xff"
    rc_a, out_a, _ = run_asm(["-t", "d1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "d1"], stdin_data=data)
    report_result(out_a == out_g, "od: -t d1 (decimal bytes) matches GNU")

    # -t u1 (unsigned decimal bytes)
    data = b"\x00\x7f\x80\xff"
    rc_a, out_a, _ = run_asm(["-t", "u1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "u1"], stdin_data=data)
    report_result(out_a == out_g, "od: -t u1 (unsigned decimal bytes) matches GNU")

    # -t a (named characters)
    data = b"\x00\x01\x07\x08\x09\x0a\x0d\x1b\x7f"
    rc_a, out_a, _ = run_asm(["-t", "a"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "a"], stdin_data=data)
    report_result(out_a == out_g, "od: -t a (named chars) matches GNU")

    # -t c (C-style characters)
    data = b"hello\tworld\n\0"
    rc_a, out_a, _ = run_asm(["-t", "c"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "c"], stdin_data=data)
    report_result(out_a == out_g, "od: -t c (C-style chars) matches GNU")

    # -b (short for -t o1)
    data = b"test"
    rc_a, out_a, _ = run_asm(["-b"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b"], stdin_data=data)
    report_result(out_a == out_g, "od: -b (octal bytes) matches GNU")

    # -c (short for -t c)
    data = b"hello\n"
    rc_a, out_a, _ = run_asm(["-c"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-c"], stdin_data=data)
    report_result(out_a == out_g, "od: -c (C chars) matches GNU")

    # -d (short for -t u2)
    data = b"\x00\x01\xff\xfe"
    rc_a, out_a, _ = run_asm(["-d"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-d"], stdin_data=data)
    report_result(out_a == out_g, "od: -d (unsigned short) matches GNU")

    # -x (short for -t x2)
    data = b"\xde\xad\xbe\xef"
    rc_a, out_a, _ = run_asm(["-x"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-x"], stdin_data=data)
    report_result(out_a == out_g, "od: -x (hex short) matches GNU")

    # -o (short for -t o2)
    data = b"\x00\x01\x02\x03"
    rc_a, out_a, _ = run_asm(["-o"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-o"], stdin_data=data)
    report_result(out_a == out_g, "od: -o (octal short) matches GNU")

    # -j N (skip bytes)
    data = b"ABCDEFGHhello\n"
    rc_a, out_a, _ = run_asm(["-j", "8"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-j", "8"], stdin_data=data)
    report_result(out_a == out_g, "od: -j 8 (skip bytes) matches GNU")

    # -N N (limit bytes)
    data = b"hello world this is a long string\n"
    rc_a, out_a, _ = run_asm(["-N", "5"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-N", "5"], stdin_data=data)
    report_result(out_a == out_g, "od: -N 5 (limit bytes) matches GNU")

    # -j and -N combined
    data = b"ABCDEFGHIJKLhello world\n"
    rc_a, out_a, _ = run_asm(["-j", "4", "-N", "8"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-j", "4", "-N", "8"], stdin_data=data)
    report_result(out_a == out_g, "od: -j 4 -N 8 combined matches GNU")

    # -w N (output width)
    data = bytes(range(64))
    rc_a, out_a, _ = run_asm(["-w16", "-t", "x1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w16", "-t", "x1"], stdin_data=data)
    report_result(out_a == out_g, "od: -w16 (output width) matches GNU")

    # -v (verbose, no duplicate suppression)
    data = b"\x00" * 64
    rc_a, out_a, _ = run_asm(["-v"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-v"], stdin_data=data)
    report_result(out_a == out_g, "od: -v (verbose) matches GNU")

    # Without -v (duplicate suppression with *)
    data = b"\x00" * 64
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "od: duplicate suppression (*) matches GNU")

    # Empty input
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "od: empty input matches GNU")

    # Single byte
    data = b"\x42"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "od: single byte matches GNU")

    # All 256 byte values
    data = bytes(range(256))
    rc_a, out_a, _ = run_asm(["-t", "x1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x1"], stdin_data=data)
    report_result(out_a == out_g, "od: all 256 byte values matches GNU")
    ...
    try:
        ...
        report_result(out_a == out_g, "od: file argument matches GNU")
    finally:
        os.unlink(tmpfile)
    ...
    rc_a, out_a, _ = run_asm(["-"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-"], stdin_data=data)
    report_result(out_a == out_g, "od: stdin via '-' matches GNU")

    # Binary data (all high bytes)
    data = bytes(range(128, 256))
    rc_a, out_a, _ = run_asm(["-t", "x1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x1"], stdin_data=data)
    report_result(out_a == out_g, "od: high byte values matches GNU")

    # Large input
    large = bytes(range(256)) * 100
    rc_a, out_a, _ = run_asm(["-t", "x1"], stdin_data=large, timeout=10)
    rc_g, out_g, _ = run_gnu(["-t", "x1"], stdin_data=large, timeout=10)
    report_result(out_a == out_g, "od: large input (25KB) matches GNU")

    # Multiple format types
    data = b"hello\n"
    rc_a, out_a, _ = run_asm(["-t", "x1", "-t", "c"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x1", "-t", "c"], stdin_data=data)
    report_result(out_a == out_g, "od: multiple -t formats matches GNU")

    # --help
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "od: --help works")

    # --version
    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "od: --version works")

    # -t x4 (hex 4-byte words)
    data = b"\xde\xad\xbe\xef\xca\xfe\xba\xbe"
    rc_a, out_a, _ = run_asm(["-t", "x4"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "x4"], stdin_data=data)
    report_result(out_a == out_g, "od: -t x4 (hex 4-byte) matches GNU")

    # -t d2 (signed short)
    data = b"\x00\x01\xff\xff"
    rc_a, out_a, _ = run_asm(["-t", "d2"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-t", "d2"], stdin_data=data)
    report_result(out_a == out_g, "od: -t d2 (signed short) matches GNU")

    # Exactly 16 bytes (one full default line)
    data = bytes(range(16))
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "od: exactly 16 bytes matches GNU")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fod) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
