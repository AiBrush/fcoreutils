...
BSS_SIZE = 65536
TOOL_NAME = "nl"
BIN = str(Path(__file__).resolve().parent.parent / "fnl")
GNU = "/usr/bin/nl"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fnl", "proc: /proc/PID/exe points to fnl")
        except Exception as e:
...
# =============================================================================
#                     13. TOOL-SPECIFIC: NL TESTS
# =============================================================================

def test_nl_specific():
    log("\n=== Nl-Specific Tests ===")
    ...
    report_result(out_a == out_g, "nl: basic numbering matches GNU")

    # Default format: 6-char right-justified number + tab + line
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    report_result(b"\t" in out_a, "nl: output contains tab separator")
    # Check right-justified number format
    first_line = out_a.split(b"\n")[0]
    report_result(first_line.strip().startswith(b"1"), "nl: line number present")

    # -b a (number all lines, including empty)
    data = b"hello\n\nworld\n"
    rc_a, out_a, _ = run_asm(["-b", "a"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "a"], stdin_data=data)
    report_result(out_a == out_g, "nl: -b a (number all) matches GNU")

    # -b t (number non-empty lines, default)
    data = b"hello\n\nworld\n"
    rc_a, out_a, _ = run_asm(["-b", "t"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "t"], stdin_data=data)
    report_result(out_a == out_g, "nl: -b t (number non-empty) matches GNU")

    # -b n (no numbering)
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-b", "n"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "n"], stdin_data=data)
    report_result(out_a == out_g, "nl: -b n (no numbering) matches GNU")

    # -n ln (left justified, no leading zeros)
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-n", "ln"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-n", "ln"], stdin_data=data)
    report_result(out_a == out_g, "nl: -n ln (left justified) matches GNU")

    # -n rn (right justified, no leading zeros, default)
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-n", "rn"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-n", "rn"], stdin_data=data)
    report_result(out_a == out_g, "nl: -n rn (right justified) matches GNU")

    # -n rz (right justified, leading zeros)
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-n", "rz"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-n", "rz"], stdin_data=data)
    report_result(out_a == out_g, "nl: -n rz (leading zeros) matches GNU")

    # -w width
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-w", "3"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w", "3"], stdin_data=data)
    report_result(out_a == out_g, "nl: -w 3 (width) matches GNU")

    # -w 10 (wider)
    rc_a, out_a, _ = run_asm(["-w", "10"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w", "10"], stdin_data=data)
    report_result(out_a == out_g, "nl: -w 10 (wide) matches GNU")

    # -s separator
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-s", ": "], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", ": "], stdin_data=data)
    report_result(out_a == out_g, "nl: -s ': ' (separator) matches GNU")

    # -s with single char
    rc_a, out_a, _ = run_asm(["-s", "|"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", "|"], stdin_data=data)
    report_result(out_a == out_g, "nl: -s '|' (single char separator) matches GNU")

    # -i increment
    data = b"line1\nline2\nline3\n"
    rc_a, out_a, _ = run_asm(["-i", "2", "-b", "a"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-i", "2", "-b", "a"], stdin_data=data)
    report_result(out_a == out_g, "nl: -i 2 (increment) matches GNU")

    # -i 5
    rc_a, out_a, _ = run_asm(["-i", "5", "-b", "a"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-i", "5", "-b", "a"], stdin_data=data)
    report_result(out_a == out_g, "nl: -i 5 (increment by 5) matches GNU")

    # -v starting number
    data = b"hello\nworld\n"
    rc_a, out_a, _ = run_asm(["-v", "10"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-v", "10"], stdin_data=data)
    report_result(out_a == out_g, "nl: -v 10 (starting number) matches GNU")

    # -v 0
    rc_a, out_a, _ = run_asm(["-v", "0"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-v", "0"], stdin_data=data)
    report_result(out_a == out_g, "nl: -v 0 (start at zero) matches GNU")

    # Stdin processing
    data = b"alpha\nbeta\ngamma\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: stdin processing matches GNU")

    # Empty input
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "nl: empty input matches GNU")

    # Only empty lines (default -b t: empty lines not numbered)
    data = b"\n\n\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: only empty lines matches GNU")

    # Mixed empty and non-empty
    data = b"hello\n\n\nworld\n\nfoo\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: mixed empty/non-empty matches GNU")

    # Single line no newline
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello")
    report_result(out_a == out_g, "nl: single line no trailing newline matches GNU")

    # Combined options: -b a -n rz -w 4 -s ". " -v 100 -i 10
    data = b"line1\n\nline3\n"
    rc_a, out_a, _ = run_asm(["-b", "a", "-n", "rz", "-w", "4", "-s", ". ", "-v", "100", "-i", "10"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "a", "-n", "rz", "-w", "4", "-s", ". ", "-v", "100", "-i", "10"], stdin_data=data)
    report_result(out_a == out_g, "nl: combined options matches GNU")
    ...
    try:
        ...
        report_result(out_a == out_g, "nl: file argument matches GNU")
    finally:
        os.unlink(tmpfile)

    # Multiple file arguments (using - for stdin)
    with tempfile.NamedTemporaryFile(mode='wb', suffix='.txt', delete=False) as f:
        f.write(b"alpha\nbeta\n")
        tmpfile = f.name
    try:
        rc_a, out_a, _ = run_asm([tmpfile], stdin_data=None)
        rc_g, out_g, _ = run_gnu([tmpfile], stdin_data=None)
        report_result(out_a == out_g, "nl: single file arg matches GNU")
    finally:
        os.unlink(tmpfile)

    # Nonexistent file
    rc_a, _, err_a = run_asm(["/nonexistent/file.txt"])
    report_result(rc_a != 0, "nl: nonexistent file returns nonzero")

    # Long lines
    long_line = b"A" * 10000 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=long_line)
    rc_g, out_g, _ = run_gnu([], stdin_data=long_line)
    report_result(out_a == out_g, "nl: very long line (10KB) matches GNU")
    ...
    report_result(out_a == out_g, "nl: 1000 lines matches GNU")

    # Tabs in content
    data = b"col1\tcol2\tcol3\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: tabs in content matches GNU")

    # Whitespace-only lines (not empty, should be numbered with -b t)
    data = b"  \n\t\nhello\n"
    rc_a, out_a, _ = run_asm(["-b", "t"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "t"], stdin_data=data)
    report_result(out_a == out_g, "nl: whitespace-only lines -b t matches GNU")

    # -p (no reset at logical pages)
    data = b"\\:\\:\\:\nhello\nworld\n\\:\\:\\:\nfoo\nbar\n"
    rc_a, out_a, _ = run_asm(["-p", "-b", "a"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-p", "-b", "a"], stdin_data=data)
    report_result(out_a == out_g, "nl: -p (no page reset) matches GNU")

    # Special characters in content
    data = b"!@#$%^&*()\n[]{}<>|\\~`\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: special characters matches GNU")

    # CRLF input
    data = b"hello\r\nworld\r\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "nl: CRLF input matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "nl: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "nl: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fnl) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
