#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for ftty.

ftty is a GNU-compatible 'tty' written in x86-64 Linux assembly.
It prints the file name of the terminal connected to standard input.
- If stdin is a terminal: prints device path (e.g. /dev/pts/0), exits 0
- If stdin is not a terminal: prints "not a tty", exits 1
...
def find_binary():
    ...
    candidate = script_dir.parent / "ftty"
    if candidate.exists():
        BIN = str(candidate)
    else:
...
def run(cmd, stdin_data=None, env=None, preexec_fn=None, timeout=None, stdin_pipe=True):
    if timeout is None:
        timeout = TIMEOUT
    ...
    try:
        out, err = p.communicate(input=stdin_data if stdin_data is not None else (b"" if stdin_pipe else None),
                                 timeout=timeout)
...
def check_syscall_surface():
    ...
    # tty uses ioctl(TIOCGPGRP) or isatty to check if stdin is a tty,
    # and may read /proc/self/fd/0 or use ttyname
    cmd = ["strace", "-f", "-e",
           "trace=%process,%network,write,read,openat,open,creat,brk,mmap,mprotect,ioctl,readlink",
           BIN]
    rc, out, err = run(cmd)
    ...
    report_result(len(all_calls) <= 15, f"syscall: total {len(all_calls)} syscalls (<=15)")
...
def check_proc_analysis():
    ...
    rc, out, err = run([BIN])
    # When piped, tty should say "not a tty" and exit 1
...
def check_fd_hygiene():
    ...
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    report_result(True, "fd: /dev/null → doesn't hang")

    # Pipe from /dev/null — stdin is not a tty
    script = f'{BIN} < /dev/null; echo $?'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    rc = p.stdout.strip().split("\n")[-1]
    report_result(rc == "1", "fd: stdin from /dev/null → exit 1 (not a tty)")
...
def check_environment():
    ...
    report_result(rc < 128, "env: 100KB env var → no signal death")
...
def check_output_integrity():
    ...
    # Compare with GNU tty (both piped → "not a tty")
...
def check_concurrency():
    ...
    for p in procs:
        try:
            p.communicate(input=b"", timeout=TIMEOUT)
            if p.returncode >= 128:
                crash_count += 1
        except subprocess.TimeoutExpired:
            p.kill()
    ...
    for _ in range(50):
        p = subprocess.Popen([BIN], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                             stdin=subprocess.PIPE)
        try:
            p.communicate(input=b"", timeout=1)
            if p.returncode < 128:
                ok_count += 1
        except subprocess.TimeoutExpired:
            p.kill()
...
def check_tool_specific():
    ...
    report_result(rc == 1, "tty: piped stdin → exit 1")
    report_result(tty_str == "not a tty", f"tty: piped stdin → output '{tty_str}' = 'not a tty'")
    ...
    if os.path.exists(GNU):
        ...
        report_result(out == out_g, f"tty: output matches GNU (ours='{tty_str}', gnu='{out_g.decode().strip()}')")
        report_result(rc == rc_g, f"tty: exit code matches GNU ({rc} vs {rc_g})")

    # -s (silent) flag — if supported, should suppress output
    if os.path.exists(GNU):
        rc_g, out_g, _ = run([GNU, "-s"])
        rc_f, out_f, _ = run([BIN, "-s"])
        report_result(rc_f == rc_g, f"tty: -s exit code matches GNU ({rc_f} vs {rc_g})")

    # Stdin from /dev/null — not a tty
    script = f'{BIN} < /dev/null'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    report_result(p.stdout.strip() == "not a tty", "tty: stdin from /dev/null → 'not a tty'")
    report_result(p.returncode == 1, "tty: stdin from /dev/null → exit 1")
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False) as f:
        ...
        tmpfile = f.name
    try:
        script = f'{BIN} < {tmpfile}'
        p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
        report_result(p.stdout.strip() == "not a tty", "tty: stdin from file → 'not a tty'")
        report_result(p.returncode == 1, "tty: stdin from file → exit 1")
    finally:
        os.unlink(tmpfile)

    # No stderr on normal operation
    rc, out, err = run([BIN])
    report_result(len(err) == 0, "tty: no stderr on normal operation")

    # Ignores stdin content
    rc, out, _ = run([BIN], stdin_data=b"fake data\n")
    report_result(rc < 128, "tty: ignores stdin content → no signal death")

    # With -- separator
    rc, _, _ = run([BIN, "--"])
    report_result(rc < 128, "tty: -- → no signal death")

    # Multiple runs produce same result
    results = [run([BIN]) for _ in range(10)]
    report_result(all(r[1] == results[0][1] for r in results), "tty: 10 runs same output")
