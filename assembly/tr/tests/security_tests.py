#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for ftr (assembly tr)."""
...
BSS_SIZE = 65536
TOOL_NAME = "tr"
BIN = str(Path(__file__).resolve().parent.parent / "ftr")
GNU = "/usr/bin/tr"
...
def test_syscall_surface():
    ...
    rc, out, err = run(["strace", "-f", "-e", "trace=%network", BIN, "a-z", "A-Z"],
    ...
    rc, out, err = run(["strace", "-f", "-e", "trace=brk,mmap,mprotect", BIN, "a-z", "A-Z"],
                       stdin_data=test_input)
...
def test_proc_runtime():
    ...
    p = subprocess.Popen([BIN, "a-z", "A-Z"], stdin=subprocess.PIPE,
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(0.05)
    try:
        pid = p.pid
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "ftr", "proc: /proc/PID/exe points to ftr")
        except Exception as e:
...
def test_memory_safety():
    ...
    for desc, data in [
        ...
        rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=data)
    ...
    big_data = (b"hello world test data\n") * 500000
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=big_data)
    report_result(rc < 128, f"mem: 10MB+ input no crash ({len(big_data)} bytes)")

    long_line = b"x" * (BSS_SIZE * 2) + b"\n"
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=long_line)
    report_result(rc < 128, "mem: single line >BSS_SIZE no crash")

    tiny_lines = b"\n" * 1000000
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=tiny_lines)
    ...
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=b"test\n" * 100, preexec_fn=limit_stack)
...
def test_signal_safety():
    ...
    script = f'seq 100 | {BIN} a-z A-Z | head -1 >/dev/null 2>/dev/null; echo $?'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    ...
    for _ in range(trials):
        rc = os.system(f'seq 100 | {BIN} a-z A-Z | head -c 1 >/dev/null 2>/dev/null')
        if rc == 0: ok_count += 1
...
def test_input_fuzzing():
    ...
    results = set()
    for _ in range(10):
        _, out, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data)
        results.add(out)
...
def test_resource_limits():
    ...
    for name, setter in [
        ...
        rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data, preexec_fn=setter)
    ...
    def combined():
        ...
        resource.setrlimit(resource.RLIMIT_STACK, (65536, 65536))
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data, preexec_fn=combined)
...
def test_environment():
    ...
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data, env={})
    ...
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data, env=hostile_env)
    report_result(rc < 128, "env: hostile environment no crash")

    large_env = {f"VAR_{i}": f"value_{i}" * 100 for i in range(1000)}
    rc, _, _ = run_asm(["a-z", "A-Z"], stdin_data=test_data, env=large_env)
...
def test_output_integrity():
    ...
    rc, out, err = run_asm(["a-z", "A-Z"], stdin_data=b"hello\n")
...
def test_error_handling():
    ...
    if which("strace"):
        rc, _, _ = run(["strace", "-e", "inject=write:error=EINTR:when=1",
                        BIN, "a-z", "A-Z"], stdin_data=b"hello\n")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: TR TESTS
# =============================================================================

def test_tr_specific():
    log("\n=== Tr-Specific Tests ===")

    # Basic a-z -> A-Z
    rc_a, out_a, _ = run_asm(["a-z", "A-Z"], stdin_data=b"hello world\n")
    rc_g, out_g, _ = run_gnu(["a-z", "A-Z"], stdin_data=b"hello world\n")
    report_result(out_a == out_g, "tr: a-z -> A-Z matches GNU")
    report_result(out_a == b"HELLO WORLD\n", "tr: a-z -> A-Z correct output")
    ...
    # Delete mode -d
    rc_a, out_a, _ = run_asm(["-d", "aeiou"], stdin_data=b"hello world\n")
    rc_g, out_g, _ = run_gnu(["-d", "aeiou"], stdin_data=b"hello world\n")
    report_result(out_a == out_g, "tr: -d delete vowels matches GNU")

    # Squeeze mode -s
    rc_a, out_a, _ = run_asm(["-s", " "], stdin_data=b"hello   world   test\n")
    rc_g, out_g, _ = run_gnu(["-s", " "], stdin_data=b"hello   world   test\n")
    report_result(out_a == out_g, "tr: -s squeeze spaces matches GNU")

    # Complement -c (or -C)
    rc_a, out_a, _ = run_asm(["-cd", "a-z\n"], stdin_data=b"Hello, World! 123\n")
    rc_g, out_g, _ = run_gnu(["-cd", "a-z\n"], stdin_data=b"Hello, World! 123\n")
    if rc_a == 0:
        report_result(out_a == out_g, "tr: -cd complement delete matches GNU")
    else:
        skip_test("tr: -cd complement", "not supported")
    ...
    for cls_from, cls_to, desc in [
        ...
        rc_a, out_a, _ = run_asm([cls_from, cls_to], stdin_data=b"Hello World\n")
        rc_g, out_g, _ = run_gnu([cls_from, cls_to], stdin_data=b"Hello World\n")
        if rc_a == 0 and rc_g == 0:
            report_result(out_a == out_g, f"tr: {desc} matches GNU")
        else:
            skip_test(f"tr: {desc}", "error or not supported")

    # Delete digits
    rc_a, out_a, _ = run_asm(["-d", "[:digit:]"], stdin_data=b"abc123def456\n")
    rc_g, out_g, _ = run_gnu(["-d", "[:digit:]"], stdin_data=b"abc123def456\n")
    if rc_a == 0 and rc_g == 0:
        report_result(out_a == out_g, "tr: -d [:digit:] matches GNU")
    else:
        skip_test("tr: -d [:digit:]", "not supported")

    # Single char translation
    rc_a, out_a, _ = run_asm(["a", "b"], stdin_data=b"aaa bbb ccc\n")
    rc_g, out_g, _ = run_gnu(["a", "b"], stdin_data=b"aaa bbb ccc\n")
    report_result(out_a == out_g, "tr: single char a->b matches GNU")

    # Octal escapes
    rc_a, out_a, _ = run_asm(["\\012", "X"], stdin_data=b"hello\nworld\n")
    rc_g, out_g, _ = run_gnu(["\\012", "X"], stdin_data=b"hello\nworld\n")
    if rc_a == 0 and rc_g == 0:
        report_result(out_a == out_g, "tr: octal \\012 matches GNU")
    else:
        skip_test("tr: octal escapes", "not supported")

    # Empty input
    rc_a, out_a, _ = run_asm(["a-z", "A-Z"], stdin_data=b"")
    rc_g, out_g, _ = run_gnu(["a-z", "A-Z"], stdin_data=b"")
    report_result(out_a == out_g, "tr: empty input matches GNU")

    # Identity translation (no change)
    data = b"hello world 123\n"
    rc_a, out_a, _ = run_asm(["a-z", "a-z"], stdin_data=data)
    report_result(out_a == data, "tr: identity a-z -> a-z")

    # Numbers and special chars preserved
    data = b"test 123 !@#\n"
    rc_a, out_a, _ = run_asm(["a-z", "A-Z"], stdin_data=data)
    report_result(b"123 !@#" in out_a, "tr: non-alpha preserved in translation")

    # Squeeze with translation
    rc_a, out_a, _ = run_asm(["-s", "a-z", "A-Z"], stdin_data=b"aabbcc\n")
    rc_g, out_g, _ = run_gnu(["-s", "a-z", "A-Z"], stdin_data=b"aabbcc\n")
    if rc_a == 0 and rc_g == 0:
        report_result(out_a == out_g, "tr: -s with translation matches GNU")
    else:
        skip_test("tr: -s with translation", "error")

    # Large input
    large = b"hello world test\n" * 10000
    rc_a, out_a, _ = run_asm(["a-z", "A-Z"], stdin_data=large, timeout=10)
    rc_g, out_g, _ = run_gnu(["a-z", "A-Z"], stdin_data=large, timeout=10)
    report_result(out_a == out_g, "tr: large input (10K lines) matches GNU")

    # Delete newlines
    rc_a, out_a, _ = run_asm(["-d", "\n"], stdin_data=b"a\nb\nc\n")
    rc_g, out_g, _ = run_gnu(["-d", "\n"], stdin_data=b"a\nb\nc\n")
    report_result(out_a == out_g, "tr: -d newlines matches GNU")

    # Binary safety
    data = bytes(range(256))
    rc_a, out_a, _ = run_asm(["a-z", "A-Z"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["a-z", "A-Z"], stdin_data=data)
    report_result(out_a == out_g, "tr: all 256 byte values matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "tr: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "tr: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (ftr) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
