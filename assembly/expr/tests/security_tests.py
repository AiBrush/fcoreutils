#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fexpr.

fexpr is a GNU-compatible 'expr' written in x86-64 Linux assembly.
...
def find_binary():
    ...
    for name in ["fexpr_release", "fexpr"]:
...
def check_fd_hygiene():
    ...
    rc, out, err = run([BIN] + ['1', '+', '1'], preexec_fn=limit_nofile)
...
def check_tool_specific():
    ...
    # Basic arithmetic
    rc, out, err = run([BIN, "1", "+", "1"])
    report_result(out.strip() == b"2" and rc == 0, "expr: 1 + 1 = 2")

    rc, out, err = run([BIN, "10", "-", "3"])
    report_result(out.strip() == b"7", "expr: 10 - 3 = 7")

    rc, out, err = run([BIN, "3", "*", "4"])
    report_result(out.strip() == b"12", "expr: 3 * 4 = 12")

    rc, out, err = run([BIN, "10", "/", "3"])
    report_result(out.strip() == b"3", "expr: 10 / 3 = 3")

    rc, out, err = run([BIN, "10", "%", "3"])
    report_result(out.strip() == b"1", "expr: 10 % 3 = 1")

    # String length
    rc, out, err = run([BIN, "length", "hello"])
    report_result(out.strip() == b"5", "expr: length hello = 5")

    # Comparison
    rc, out, err = run([BIN, "5", "=", "5"])
    report_result(out.strip() == b"1" and rc == 0, "expr: 5 = 5 is true")

    rc, out, err = run([BIN, "5", "=", "6"])
    report_result(rc == 1, "expr: 5 = 6 is false (exit 1)")

    # Zero result
    rc, out, err = run([BIN, "0"])
    report_result(rc == 1, "expr: 0 exits 1")

    # Missing operand
    rc, out, err = run([BIN])
    report_result(rc != 0, "expr: missing operand exits non-zero")
