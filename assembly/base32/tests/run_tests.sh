#!/bin/bash
# Test suite for fbase32
# Usage: bash tests/run_tests.sh ./fbase32
...
# ── Setup temp files ─────────────────────────────────────────
TMPDIR=$(mktemp -d /tmp/b32_test.XXXXXX)
trap "rm -rf $TMPDIR" EXIT

printf "hello world\n" > "$TMPDIR/basic.txt"
printf "NBSWY3DPEB3W64TMMQQQ====\n" > "$TMPDIR/encoded.txt"
...
run_test_stdin "decode NBSWY3DP" "NBSWY3DP" -d
run_test_stdin "decode JBSWY3DPEHPK3PXP" "JBSWY3DPEHPK3PXP" -d
...
run_test_stdin "decode multiline encoded" "NBSW\nY3DP" -d
