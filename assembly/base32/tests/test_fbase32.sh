...
echo ""
echo "============================================"
echo " GNU Compatibility Tests: fbase32"
echo "============================================"
echo ""
...
base32 "$TMPDIR/binary.bin" > "$TMPDIR/encoded.txt"
base32 -w0 "$TMPDIR/binary.bin" > "$TMPDIR/encoded_nowrap.txt"
...
run_test "wrap 5"                      "Hello World" -w5
run_test "wrap 7"                      "Hello World" -w7
run_test "wrap 8"                      "Hello World" -w8
...
echo ""
echo "── Decoding ──"
run_test_binary "decode: basic"               "JBSWY3DPEHPK3PXP" -d
run_test_binary "decode: with newlines"        "JBSWY3DP
EHPK3PXP" -d
run_test_binary "decode: empty"                "" -d
run_test_binary "decode: 6 pad"                "MY======" -d
run_test_binary "decode: 4 pad"                "MZXQ====" -d
run_test_binary "decode: 3 pad"                "MZXW6===" -d
run_test_binary "decode: 1 pad"                "MZXW6YQ=" -d
run_test_binary "decode: no pad"               "MZXW6YTB" -d
run_test_binary "decode: multiline"            "$(base32 "$TMPDIR/small.bin")" -d
...
run_test_binary "decode -i: garbage chars"     "JBS@@WY3DP@@EHPK3PXP" -di
run_test_binary "decode -i: lots of garbage"   "M!!!Z!!!X!!!W!!!6===!!!" -di
...
run_test_binary "decode -d -i"         "JBS@@WY3DP" -d -i
run_test_binary "decode -d --ignore-garbage" "JBS@@WY3DP" -d --ignore-garbage
...
run_test "incomplete decode"           "MY" -d
...
for size in 0 1 2 3 4 5 10 100 1000 10000; do
    ...
    encoded=$(base32 "$TMPDIR/rt_$size")
    ...
    printf '%s' "$our_encoded" | base32 -d > "$TMPDIR/rt_decoded2_$size" 2>/dev/null
...
diff <(base32 "$TMPDIR/large.bin") <($TOOL "$TMPDIR/large.bin") > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
diff "$TMPDIR/large.bin" <(base32 "$TMPDIR/large.bin" | $TOOL -d) > /dev/null 2>&1
if [ $? -eq 0 ]; then
