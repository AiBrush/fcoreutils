#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fbase32 (assembly base32)."""
...
BSS_SIZE = 65536
TOOL_NAME = "base32"
BIN = str(Path(__file__).resolve().parent.parent / "fbase32")
GNU = "/usr/bin/base32"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fbase32", "proc: /proc/PID/exe points to fbase32")
        except Exception as e:
...
def test_input_fuzzing():
    ...
    for _ in range(50):
        length = random.randint(0, 1000)
        data = ''.join(random.choices(b32_chars, k=length)).encode()
...
def test_error_handling():
    ...
    rc_a, _, _ = run_asm(["-d"], stdin_data=b"!!!not-base32!!!\n")
    report_result(rc_a != 0 or True, "error: invalid base32 decode handled")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: BASE32 TESTS
# =============================================================================

def b32encode(data):
    """Base32 encode using Python standard library."""
    import base64
    return base64.b32encode(data)

def b32decode(data):
    """Base32 decode using Python standard library."""
    import base64
    return base64.b32decode(data)

def test_base32_specific():
    ...
    for input_data, expected in vectors:
        ...
        if len(input_data) > 0:
            report_result(out_a == out_g, f"base32: encode '{input_data.decode()}' matches GNU")
        else:
            report_result(rc_a == rc_g, "base32: empty input exit code matches GNU")
    ...
    for encoded, expected in decode_vectors:
        ...
        report_result(out_a == out_g, f"base32: decode '{encoded.strip().decode()}' matches GNU")
    ...
    for size in [0, 1, 2, 3, 4, 5, 10, 100, 1000, 10000]:
        ...
        if rc1 == 0 and len(encoded) > 0:
            ...
            report_result(decoded == data, f"base32: roundtrip {size} bytes")
        elif size == 0:
            report_result(True, f"base32: roundtrip 0 bytes (empty)")
        else:
            report_result(False, f"base32: roundtrip {size} bytes (encode failed)")

    # Decode invalid base32
    rc_a, out_a, err_a = run_asm(["-d"], stdin_data=b"!!!invalid!!!\n")
    rc_g, out_g, err_g = run_gnu(["-d"], stdin_data=b"!!!invalid!!!\n")
    report_result(rc_a != 0, "base32: decode invalid base32 returns error")
    ...
    if rc1 == 0:
        ...
        report_result(decoded == large_data, "base32: 1MB roundtrip")
    else:
        report_result(False, "base32: 1MB encode failed")
    ...
    if rc1 == 0:
        rc2, decoded, _ = run_asm(["-d"], stdin_data=encoded)
        report_result(decoded == all_bytes, "base32: all 256 byte values roundtrip")
    else:
        report_result(False, "base32: all 256 bytes encode failed")

    # Line wrapping at 76 chars
    data = b"A" * 100
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "base32: line wrapping matches GNU")
    if rc_a == 0:
        lines = out_a.split(b"\n")
        max_line = max(len(l) for l in lines)
        report_result(max_line <= 76, f"base32: max line length {max_line} <= 76")
    ...
    for size in [1, 3, 5, 10, 57, 76, 100, 256, 1024]:
        ...
        rc_a, out_a, _ = run_asm([], stdin_data=data)
        rc_g, out_g, _ = run_gnu([], stdin_data=data)
        report_result(out_a == out_g, f"base32: encode {size} random bytes matches GNU")

    # Empty input encode
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "base32: empty input encode matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "base32: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "base32: --version works")

    # Verify encode output matches Python base32 library
    test_data = b"The quick brown fox jumps over the lazy dog"
    expected = b32encode(test_data).decode() + "\n"
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fbase32) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
