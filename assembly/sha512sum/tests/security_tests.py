#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fsha512sum (assembly sha512sum).

fsha512sum is a GNU-compatible 'sha512sum' written in x86-64 Linux assembly.
It computes SHA-512 message digests.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (sha512sum: SHA-512 checksum behavior)
"""
...
BSS_SIZE = 65536
TOOL_NAME = "sha512sum"
BIN = str(Path(__file__).resolve().parent.parent / "fsha512sum")
...
def sha512(data):
    """Compute SHA-512 hash of data using Python hashlib."""
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fsha512sum", "proc: /proc/PID/exe points to fsha512sum")
        except Exception as e:
...
# =============================================================================
#                     13. TOOL-SPECIFIC: SHA512SUM TESTS
# =============================================================================

def test_sha512sum_specific():
    ...
    for data, expected_hash in vectors:
        ...
        if rc == 0:
            ...
            report_result(False, f"sha512: vector failed (rc={rc})")
    ...
    for data, expected_hash in vectors:
        ...
        if rc_a == 0 and rc_g == 0:
            ...
            report_result(hash_a == hash_g, f"sha512: GNU match '{data[:20].decode(errors='replace')}...'")
    ...
    if rc == 0:
        ...
        parts = out_str.split()
        report_result(len(parts) >= 2 and len(parts[0]) == 128 and parts[1] == "-",
                     "sha512: output format 'hash  -'")
    else:
        report_result(False, "sha512: output format check (command failed)")

    # Compare with GNU output format
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "sha512: output format matches GNU exactly")

    # Binary data hashing (all 256 byte values)
    all_bytes = bytes(range(256))
    expected = sha512(all_bytes)
    rc, out, _ = run_asm([], stdin_data=all_bytes)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha512: all 256 byte values hash correct")

    # Large file hashing (1MB+)
    large_data = os.urandom(1024 * 1024)
    expected = sha512(large_data)
    rc, out, _ = run_asm([], stdin_data=large_data, timeout=15)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha512: 1MB hash correct")
    else:
        report_result(False, "sha512: 1MB hash (command failed)")
    ...
    for i in range(20):
        ...
        if rc == 0:
            output_hash = out.decode().strip().split()[0] if out else ""
            report_result(output_hash == expected, f"sha512: random {size} bytes hash correct")
        else:
            report_result(False, f"sha512: random {size} bytes (command failed)")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        report_result(out_a == out_g and rc_a == rc_g, "sha512: file hash matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha512sum: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha512sum: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fsha512sum) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
