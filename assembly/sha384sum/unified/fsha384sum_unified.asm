; fsha384sum_unified.asm
; Hand-crafted minimal ELF binary for fsha384sum
; Fully self-contained — no libc, no linker needed
; Build: nasm -f bin fsha384sum_unified.asm -o fsha384sum && chmod +x fsha384sum
;
...
; Output: first 384 bits (48 bytes, 96 hex chars) of the hash state.
...
; SHA-384 constants
%define HASH_HEX_LEN  96      ; 384 bits = 48 bytes = 96 hex chars
%define HASH_QWORDS    6      ; 6 qwords to output
...
; ── SHA-384 Init ──
; SHA-384 uses different initial hash values than SHA-512 (FIPS 180-4 section 5.3.4)
sha384_init:
    mov rax, 0xcbbb9d5dc1059ed8
    mov [hash_state], rax
    mov rax, 0x629a292a367cd507
    mov [hash_state+8], rax
    mov rax, 0x9159015a3070dd17
    mov [hash_state+16], rax
    mov rax, 0x152fecd8f70e5939
    mov [hash_state+24], rax
    mov rax, 0x67332667ffc00b31
    mov [hash_state+32], rax
    mov rax, 0x8eb44a8768581511
    mov [hash_state+40], rax
    mov rax, 0xdb0c2e0d64f98fa7
    mov [hash_state+48], rax
    mov rax, 0x47b5481dbefa4fa4
...
; ── SHA-512 Transform (shared with SHA-384) ──
...
; ── SHA-384 Update (same as SHA-512 update) ──
sha384_update:
    push    rbx
    push    r12
    push    r13
...
; ── SHA-384 Final (same as SHA-512 final) ──
sha384_final:
    push    rbx
    mov     eax, [block_used]
...
; ── SHA-384 to hex (96 hex chars — first 6 qwords) ──
; Hash state stores values as native 64-bit integers.
; We extract nibbles MSB-first using shifts and masks.
sha384_to_hex:
    xor     ecx, ecx
...
.loop:
    ...
    ; Convert 8 bytes to 16 hex chars (MSB first)
    push    rcx
    push    r9
    lea     rdx, [hex_out]
    add     rdx, r9         ; rdx = hex_out + offset
    ; Extract nibbles from MSB to LSB
    mov     ecx, 0          ; position in output
.hex_nibble:
    cmp     ecx, 16
    jge     .hex_done
    mov     rbx, rax
    ; Shift right to get the desired nibble at the bottom
    ; For position 0: shift right by 60
    ; For position 1: shift right by 56
    ; For position k: shift right by (60 - k*4) = (15-k)*4
    push    rcx
    mov     r8d, 15
    sub     r8d, ecx
    shl     r8d, 2          ; r8d = (15-ecx)*4
    mov     cl, r8b
    shr     rbx, cl
    pop     rcx
    and     rbx, 0x0F
...
.output_tag:
    mov     rdi, out_buf
    mov     rsi, str_sha384_tag
    mov     ecx, str_sha384_tag_len
    rep     movsb
    mov     rsi, r12
.tcf: lodsb
    test    al, al
...
.have_line:
    ...
    inc     rsi
.no_esc:
    ; Try BSD-style tag format: SHA384 (filename) = hash
    cmp     byte [rsi], 'S'
    jne     .try_std
    cmp     byte [rsi+1], 'H'
    jne     .try_std
...
; SHA-512 round constants K[0..79] (shared with SHA-384)
sha512_K:
...
str_help:
    db "Usage: sha384sum [OPTION]... [FILE]...", 10
    db "Print or check SHA384 (384-bit) checksums.", 10
    db 10
    ...
    db "Full documentation <https://www.gnu.org/software/coreutils/sha384sum>", 10
    db "or available locally via: info '(coreutils) sha384sum invocation'", 10
str_help_len equ $ - str_help

str_version:
    db "sha384sum (GNU coreutils) 9.7", 10
...
str_sha384_tag: db "SHA384 ("
str_sha384_tag_len equ $ - str_sha384_tag
str_tag_eq: db ") = "
...
err_unrec: db "sha384sum: unrecognized option '"
err_unrec_len equ $ - err_unrec
err_inval: db "sha384sum: invalid option -- '"
err_inval_len equ $ - err_inval
err_suffix:   db 0xE2, 0x80, 0x99, 10
err_suffix_len equ $ - err_suffix

err_tag_check: db "sha384sum: the --tag option is meaningless when verifying checksums", 10
               db "Try 'sha384sum --help' for more information.", 10
err_tag_check_len equ $ - err_tag_check

str_warn_prefix: db "sha384sum: WARNING: "
...
str_no_proper: db ": no properly formatted SHA384 checksum lines found", 10
...
str_improperly: db ": improperly formatted SHA384 checksum line", 10
