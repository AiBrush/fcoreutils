#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fdd.

fdd is a GNU-compatible 'dd' written in x86-64 Linux assembly.
It copies and converts files.
...
def check_proc_analysis():
    log("\n=== 3. /proc Filesystem Runtime Analysis ===")
    rc, out, err = run([BIN] + ['if=/dev/null'], stdin_data=b'test')
...
def check_fd_hygiene():
    ...
    rc, out, err = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_nofile)
...
def check_memory_safety():
    ...
    for i in range(10):
        rc, _, _ = run([BIN] + ['if=/dev/null'], stdin_data=b'test')
        if rc >= 128:
    ...
    rc, out, err = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_stack)
    report_result(rc >= 0 and rc < 128, "memory: 64KB stack — no crash")

    def limit_mem():
        resource.setrlimit(resource.RLIMIT_AS, (16 * 1024 * 1024, 16 * 1024 * 1024))
    rc, out, err = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_mem)
...
def check_resource_limits():
    ...
    def limit_as():
        resource.setrlimit(resource.RLIMIT_AS, (16 * 1024 * 1024, 16 * 1024 * 1024))
    rc, _, _ = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_as)
    report_result(rc >= 0 and rc < 128, "rlimit: RLIMIT_AS=16MB — no crash")

    def limit_nofile():
        resource.setrlimit(resource.RLIMIT_NOFILE, (3, 3))
    rc, _, _ = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_nofile)
    report_result(rc >= 0 and rc < 128, "rlimit: RLIMIT_NOFILE=3 — no crash")

    def limit_cpu():
        resource.setrlimit(resource.RLIMIT_CPU, (1, 1))
    rc, _, _ = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_cpu)
    report_result(rc >= 0 and rc < 128, "rlimit: RLIMIT_CPU=1s — no crash")

    def limit_stack():
        resource.setrlimit(resource.RLIMIT_STACK, (65536, 65536))
    rc, _, _ = run([BIN] + ['if=/dev/null'], stdin_data=b'test', preexec_fn=limit_stack)
...
def check_concurrency():
    ...
    procs = []
    for _ in range(50):
        p = subprocess.Popen([BIN] + ['if=/dev/null'],
                           stdin=subprocess.PIPE if True else None,
    ...
    for p in procs:
        try:
            out, err = p.communicate(input=b"b'test'", timeout=TIMEOUT)
            if p.returncode >= 128:
                crash_count += 1
        except subprocess.TimeoutExpired:
            p.kill()
...
def check_tool_specific():
    ...
    # Basic copy
    rc, out, err = run([BIN, "if=/dev/null"], stdin_data=b"hello\n")
    report_result(rc == 0, "dd: if=/dev/null exits 0")

    # Stats output to stderr
    rc, out, err = run([BIN], stdin_data=b"hello\n")
    report_result(b"records in" in err or b"records" in err, "dd: stats written to stderr")

    # status=none suppresses stderr
    rc, out, err = run([BIN, "status=none"], stdin_data=b"hello\n")
    report_result(err == b"" or len(err) == 0, "dd: status=none suppresses stderr")

    # Copy stdin to stdout
    rc, out, err = run([BIN, "status=none"], stdin_data=b"test data\n")
    report_result(out == b"test data\n", "dd: stdin passes to stdout")

    # Empty input
    rc, out, err = run([BIN, "status=none"], stdin_data=b"")
    report_result(out == b"" and rc == 0, "dd: empty input")

    # Large data
    data = b"X" * 100000
    rc, out, err = run([BIN, "status=none"], stdin_data=data)
    report_result(out == data, "dd: 100KB passthrough")
