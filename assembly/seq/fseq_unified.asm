; ============================================================
; fseq_unified.asm — Unified flat-binary seq implementation
...
; ============================================================================
;  Library: asm_write_all, asm_write, asm_exit, asm_strlen, asm_write_err
; ============================================================================
...
_start:
    ...
    mov     qword [num_ints], 0
    mov     qword [num_ints+8], 0
    mov     qword [num_ints+16], 0
    mov     qword [first_val], 0
    mov     qword [incr_val], 0
    mov     qword [last_val], 0
...
.one_arg:
    ...
    mov     qword [first_val], 1
    mov     qword [incr_val], 1
    mov     rax, [num_ints]
    mov     [last_val], rax
    mov     rax, [num_strs]
    mov     [last_str], rax
...
.error_missing_operand:
    lea     rsi, [err_missing_operand]
    mov     edx, err_missing_operand_len
    jmp     .print_err_exit

.error_extra_operand:
    ; "seq: extra operand 'X'"
    lea     rsi, [err_extra_operand]
...
.error_zero_incr:
    ...
    mov     rbx, [num_strs+8]       ; incr string
    lea     rsi, [err_zero_incr_pre]
...
.error_missing_fmt:
    lea     rsi, [err_missing_fmt]
    mov     edx, err_missing_fmt_len
    jmp     .print_err_exit

.error_missing_sep:
    lea     rsi, [err_missing_sep]
...
seq_integer:
    ...
    mov     r12, [first_val]        ; current value
    mov     r13, [incr_val]         ; increment
    mov     r14, [last_val]         ; last value
...
.int_check_fastpath:
    ...
    lea     rdi, [itoa_buf + ITOA_BUF_SIZE - 1]
    mov     byte [rdi], 0
    test    rax, rax
    jnz     .fast_init_loop
    dec     rdi
...
.fast_init_copy:
    lea     rcx, [itoa_buf + ITOA_BUF_SIZE - 1]
    sub     rcx, rdi
    mov     r13, rcx
    xor     eax, eax
.fast_init_cp:
...
.fast_init_done:
    ...
    lea     rbx, [outbuf]
    add     rbx, [outbuf_pos]
    lea     rbp, [outbuf + OUTBUF_SIZE - 2200]
...
.fast_try_b10:
    ...
    lea     rax, [outbuf + OUTBUF_SIZE - 256]
    cmp     rbx, rax
    jae     .fast_main_flush
    jmp     .fast_do_b10
...
.fast_do_b100:
    ...
    lea     rsi, [digit_pairs]      ; pairs table
...
emit_integer:
    ...
    lea     rdi, [itoa_buf + ITOA_BUF_SIZE]
    mov     rsi, r12
    call    itoa_reverse
...
seq_float:
    ...
    mov     rdi, [first_str]
    call    parse_double
    movsd   [first_fval], xmm0

    mov     rdi, [incr_str]
    call    parse_double
    movsd   [incr_fval], xmm0

    mov     rdi, [last_str]
    call    parse_double
    movsd   [last_fval], xmm0
    ...
    movsd   xmm0, [first_fval]
    mov     edi, r12d
    call    float_format_width
    mov     ebx, eax

    movsd   xmm0, [last_fval]
    mov     edi, r12d
    call    float_format_width
    cmp     eax, ebx
    cmovg   ebx, eax
...
.float_loop_start:
    ...
    movsd   xmm1, [incr_fval]      ; increment
    movsd   xmm2, [last_fval]      ; last
...
.float_neg_loop:
    ...
    ; current += increment
    movsd   xmm0, [rsp]
    addsd   xmm0, [incr_fval]
    movsd   [rsp], xmm0
    movsd   xmm2, [last_fval]
    jmp     .float_neg_loop
...
emit_float_fmt:
    ...
    mov     r12, [fmt_ptr]          ; format string
    mov     r13, [fmt_len]          ; format length
...
.eff_space_ok:
    ...
    lea     rbp, [outbuf]
    add     rbp, [outbuf_pos]       ; rbp = write pointer
...
.pd_check_dot:
    ...
    movsd   xmm3, [const_tenth]     ; current place value
.pd_frac_loop:
    movzx   eax, byte [r12 + rbx]
    sub     eax, '0'
    cmp     eax, 9
...
; All BSS variables are defined as offsets from the end of file content

; We need alignment for some vars, so compute carefully
bss_base equ $$ + file_size

; 8-byte aligned variables
argc        equ bss_base
argv        equ argc + 8
end_of_opts equ argv + 8
flag_w      equ end_of_opts + 1
flag_f      equ flag_w + 1
is_float    equ flag_f + 1
; pad to 4-byte boundary
num_count   equ is_float + 1
float_prec  equ num_count + 4

; Number storage (3 values max)
num_strs    equ float_prec + 4          ; 3 * 8 = 24 bytes
num_ints    equ num_strs + 24           ; 3 * 8 = 24 bytes

; Resolved values
first_val   equ num_ints + 24
incr_val    equ first_val + 8
last_val    equ incr_val + 8

first_str   equ last_val + 8
incr_str    equ first_str + 8
last_str    equ incr_str + 8

; Float values
first_fval  equ last_str + 8
incr_fval   equ first_fval + 8
last_fval   equ incr_fval + 8

; Separator
sep_ptr     equ last_fval + 8
sep_len     equ sep_ptr + 8

; Format
fmt_ptr     equ sep_len + 8
fmt_len     equ fmt_ptr + 8

; emit_float_fmt temporaries
_eff_flags  equ fmt_len + 8
_eff_width  equ _eff_flags + 4
_eff_prec   equ _eff_width + 4

; Output buffer
outbuf_pos  equ _eff_prec + 4
itoa_buf    equ outbuf_pos + 8
fmt_scratch equ itoa_buf + ITOA_BUF_SIZE + 8
digit_buf   equ fmt_scratch + FMT_BUF_SIZE + 8

; Main output buffer
outbuf      equ digit_buf + 32

bss_size    equ outbuf + OUTBUF_SIZE - bss_base
