; ============================================================
; fchgrp_unified.asm -- GNU-compatible 'chgrp' command
; Builds with: nasm -f bin fchgrp_unified.asm -o fchgrp
;
...
;   r13d = file arg index
;   g_target_gid = target gid (global, writable in RWX segment)
;
; Flags in ebx:
;   bit 0 = -v (verbose)
...
.done_opts:
    ...
    cmp     ecx, r14d
    jge     .err_missing_operand
    mov     r13d, ecx           ; save group arg index
    ...
    mov     rdi, [r15 + r13*8]
    call    lookup_group
    ; eax = gid+1 on success, 0 on failure
    test    eax, eax
    jz      .err_invalid_group
    dec     eax                 ; convert to actual gid
...
.err_invalid_group:
    ...
    mov     edx, str_prefix_len
    call    do_write_err
    mov     rsi, str_invalid_group
    mov     edx, str_invalid_group_len
    call    do_write_err
    mov     rdi, [r15 + r13*8]
    call    str_len
    mov     edx, eax
...
; ============================================================
; do_chgrp_file: chgrp a single file, recurse if -R
; Input: rdi = path, ebx = flags, ebp = exit code
; ============================================================
do_chgrp_file:
    push    rbx
    push    r12
...
.cf_stat:
    ...
    mov     r12d, [rsp + STAT_GID]  ; old gid
    mov     r14d, [g_target_gid]    ; new gid

    ; chown(path, -1, gid)
    mov     rdi, r15
    mov     esi, -1                  ; uid unchanged
    mov     edx, r14d                ; new gid
    test    r13d, 16
    jnz     .cf_lchown
    mov     eax, SYS_CHOWN
    syscall
...
.cf_verbose:
    ...
    mov     rsi, str_changed_group
    mov     edx, str_changed_group_len
...
.cf_retained:
    mov     rsi, str_group_of
    mov     edx, str_group_of_len
    call    do_write_stdout
    mov     rdi, r15
    call    str_len
    mov     edx, eax
...
.cf_chown_err:
    ...
    mov     edx, str_prefix_len
    call    do_write_err
    mov     rsi, str_changing_group
    mov     edx, str_changing_group_len
    call    do_write_err
    mov     rdi, r15
    call    str_len
    mov     edx, eax
...
; This returns gid+1 so that gid 0 can be distinguished from not-found.
; Caller must test != 0, then dec eax.
; ============================================================
lookup_group:
    push    rbx
    push    r12
...
; ============================================================
; match_group: rdi=line, rsi=name
; Returns eax = gid+1 on match, 0 on no match
; Line format: groupname:x:gid:members
; ============================================================
match_group:
    push    rbx
    xor     ecx, ecx
...
.mg_parse:
    ...
    jmp     .mg_parse
.mg_done:
    inc     eax                 ; gid+1 so that gid 0 returns 1
    pop     rbx
    ret
.mg_no:
    xor     eax, eax
...
; ============================================================
; print_uint: edi = value, prints to stdout
; ============================================================
print_uint:
    sub     rsp, 24
    lea     rcx, [rsp + 20]
...
str_help:
    db "Usage: chgrp [OPTION]... GROUP FILE...", 10
    db "  or:  chgrp [OPTION]... --reference=RFILE FILE...", 10
    db "Change the group of each FILE to GROUP.", 10
    db "With --reference, change the group of each FILE to that of RFILE.", 10, 10
    ...
    db "      --reference=RFILE  use RFILE's group rather than specifying a", 10
    db "                         GROUP value.  RFILE is always dereferenced.", 10
    db "  -R, --recursive        operate on files and directories recursively", 10
    db "      --help        display this help and exit", 10
    db "      --version     output version information and exit", 10, 10
    db "Examples:", 10
    db '  chgrp staff /u      Change the group of /u to "staff".', 10
    db '  chgrp -hR staff /u  Change the group of /u and subfiles to "staff".', 10, 10
    db "GNU coreutils online help: <https://www.gnu.org/software/coreutils/>", 10
    db "Full documentation <https://www.gnu.org/software/coreutils/chgrp>", 10
    db "or available locally via: info '(coreutils) chgrp invocation'", 10
str_help_len equ $ - str_help

str_version:
    db "chgrp (GNU coreutils) 9.7", 10
...
str_sq_nl:          db "'", 10
str_try:            db "Try 'chgrp --help' for more information.", 10
str_try_len         equ $ - str_try
str_colon_sep:      db "': "
str_colon_sep_len   equ $ - str_colon_sep
str_cannot_access:  db "cannot access '"
str_cannot_access_len equ $ - str_cannot_access
str_changing_group: db "changing group of '"
str_changing_group_len equ $ - str_changing_group
str_failed_ref:     db "failed to get attributes of '"
str_failed_ref_len  equ $ - str_failed_ref
str_invalid_group:  db "invalid group: '"
str_invalid_group_len equ $ - str_invalid_group
str_changed_group:  db "changed group of '"
str_changed_group_len equ $ - str_changed_group
str_group_of:       db "group of '"
str_group_of_len    equ $ - str_group_of
str_sq_from:        db "' from "
