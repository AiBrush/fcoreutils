#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fsha1sum (assembly sha1sum).

fsha1sum is a GNU-compatible 'sha1sum' written in x86-64 Linux assembly.
It computes SHA-1 message digests.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (sha1sum: SHA-1 checksum behavior)
"""
...
BSS_SIZE = 65536
TOOL_NAME = "sha1sum"
BIN = str(Path(__file__).resolve().parent.parent / "fsha1sum")
GNU = "/usr/bin/sha1sum"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fsha1sum", "proc: /proc/PID/exe points to fsha1sum")
        except Exception as e:
...
def test_sha1sum_specific():
    ...
    for data, expected_hash in vectors:
        ...
        if rc == 0:
            ...
            report_result(False, f"sha1: vector failed (rc={rc})")
    ...
    for data, expected_hash in vectors:
        ...
        if rc_a == 0 and rc_g == 0:
            ...
            report_result(hash_a == hash_g, f"sha1: GNU match '{data[:20].decode(errors='replace')}...'")
    ...
    if rc == 0:
        ...
        parts = out_str.split()
        report_result(len(parts) >= 2 and len(parts[0]) == 40 and parts[1] == "-",
                     "sha1: output format 'hash  -'")
    else:
        report_result(False, "sha1: output format check (command failed)")

    # Compare with GNU output format
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "sha1: output format matches GNU exactly")
    ...
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha1: all 256 byte values hash correct")
    ...
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha1: 1MB hash correct")
    else:
        report_result(False, "sha1: 1MB hash (command failed)")
    ...
    for i in range(20):
        ...
        if rc == 0:
            output_hash = out.decode().strip().split()[0] if out else ""
            report_result(output_hash == expected, f"sha1: random {size} bytes hash correct")
        else:
            report_result(False, f"sha1: random {size} bytes (command failed)")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        report_result(out_a == out_g and rc_a == rc_g, "sha1: file hash matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha1sum: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha1sum: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fsha1sum) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
