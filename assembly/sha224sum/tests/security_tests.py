#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fsha224sum (assembly sha224sum)."""
...
BSS_SIZE = 65536
TOOL_NAME = "sha224sum"
BIN = str(Path(__file__).resolve().parent.parent / "fsha224sum")
...
def sha224(data):
    """Compute SHA-224 hash of data using Python hashlib."""
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fsha224sum", "proc: /proc/PID/exe points to fsha224sum")
        except Exception as e:
...
# =============================================================================
#                     13. TOOL-SPECIFIC: SHA224SUM TESTS
# =============================================================================

def test_sha224sum_specific():
    ...
    for data, expected_hash in vectors:
        ...
        if rc == 0:
            ...
            report_result(False, f"sha224: vector failed (rc={rc})")
    ...
    for data, expected_hash in vectors:
        ...
        if rc_a == 0 and rc_g == 0:
            ...
            report_result(hash_a == hash_g, f"sha224: GNU match '{data[:20].decode(errors='replace')}...'")
    ...
    if rc == 0:
        ...
        parts = out_str.split()
        report_result(len(parts) >= 2 and len(parts[0]) == 56 and parts[1] == "-",
                     "sha224: output format 'hash  -'")
    else:
        report_result(False, "sha224: output format check (command failed)")

    # Compare with GNU output format
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "sha224: output format matches GNU exactly")

    # Binary data hashing (all 256 byte values)
    all_bytes = bytes(range(256))
    expected = sha224(all_bytes)
    rc, out, _ = run_asm([], stdin_data=all_bytes)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha224: all 256 byte values hash correct")

    # Large file hashing (1MB+)
    large_data = os.urandom(1024 * 1024)
    expected = sha224(large_data)
    rc, out, _ = run_asm([], stdin_data=large_data, timeout=15)
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "sha224: 1MB hash correct")
    else:
        report_result(False, "sha224: 1MB hash (command failed)")

    # Random data hashing accuracy (compare with Python hashlib)
    for i in range(20):
        size = random.randint(1, 10000)
        data = os.urandom(size)
        expected = sha224(data)
        rc, out, _ = run_asm([], stdin_data=data)
        if rc == 0:
            output_hash = out.decode().strip().split()[0] if out else ""
            report_result(output_hash == expected, f"sha224: random {size} bytes hash correct")
        else:
            report_result(False, f"sha224: random {size} bytes (command failed)")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha224sum: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "sha224sum: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fsha224sum) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
