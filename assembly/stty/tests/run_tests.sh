#!/bin/bash
# Test suite for fstty
# Usage: bash tests/run_tests.sh ./fstty
...
# ── No TTY (piped stdin) — should fail identically ──
desc="no tty (piped stdin)"
echo "" | $GNU 2>/dev/null
expected_exit=$?
echo "" | $BIN 2>/dev/null
got_exit=$?
...
# ── -a flag (no tty) ──
desc="-a flag no tty"
echo "" | $GNU -a 2>/dev/null
expected_exit=$?
echo "" | $BIN -a 2>/dev/null
got_exit=$?
if [ "$expected_exit" = "$got_exit" ]; then
    PASS=$((PASS+1))
else
...
# ── -g flag (no tty) ──
desc="-g flag no tty"
echo "" | $GNU -g 2>/dev/null
expected_exit=$?
echo "" | $BIN -g 2>/dev/null
got_exit=$?
if [ "$expected_exit" = "$got_exit" ]; then
    PASS=$((PASS+1))
else
