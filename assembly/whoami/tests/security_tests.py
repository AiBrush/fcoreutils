...
fwhoami is a GNU-compatible 'whoami' written in x86-64 Linux assembly.
It prints the effective user name associated with the current effective user ID.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (whoami behavior)
"""
...
def find_binary():
    ...
    candidate = script_dir.parent / "fwhoami"
    if candidate.exists():
        BIN = str(candidate)
    else:
...
def check_syscall_surface():
    ...
    # whoami needs: geteuid + read /etc/passwd or direct lookup + write + exit_group
    cmd = ["strace", "-f", "-e",
           "trace=%process,%network,write,read,openat,open,creat,brk,mmap,mprotect,geteuid",
           BIN]
    rc, out, err = run(cmd)
    ...
    report_result(len(all_calls) <= 20, f"syscall: total {len(all_calls)} syscalls (<=20)")
...
def check_environment():
    ...
    hostile = {"PATH": "", "HOME": "/nonexistent", "LANG": "xx_XX.BROKEN",
               "TERM": "", "USER": "evil_user", "LOGNAME": "evil"}
    ...
    special_env["USER"] = "A" * 100000
    rc, _, _ = run([BIN], env=special_env)
    report_result(rc < 128, "env: 100KB USER env var → no signal death")
...
def check_error_handling():
    ...
    if os.path.exists(GNU):
        ...
        # Extra arg
        rc_f, _, _ = run([BIN, "extra"])
        rc_g, _, _ = run([GNU, "extra"])
...
def check_tool_specific():
    ...
    if os.path.exists(GNU):
        ...
        report_result(name == gnu_name,
                      f"whoami: '{name}' matches GNU '{gnu_name}'")

    # Exactly one line
    lines = out.decode(errors="replace").split("\n")
    non_empty = [l for l in lines if l]
    report_result(len(non_empty) == 1, "whoami: exactly one line of output")

    # No stderr
    report_result(len(err) == 0, "whoami: no stderr output")

    # Valid username format
    import re
    valid = re.match(r'^[a-zA-Z0-9_][\-a-zA-Z0-9_.]*$', name)
    report_result(valid is not None, f"whoami: '{name}' is valid username format")

    # Ignores arguments
    rc, out2, _ = run([BIN, "ignored"])
    report_result(rc < 128, "whoami: with extra arg → no signal death")

    # Multiple runs same result
    results = [run([BIN])[1] for _ in range(10)]
    report_result(all(r == results[0] for r in results), "whoami: 10 runs same output")

    # With -- separator
    rc, _, _ = run([BIN, "--"])
    report_result(rc < 128, "whoami: -- → no signal death")
    ...
    if rc == 0:
        report_result(out.decode().strip() == effective_user,
