...
fsync is a GNU-compatible 'sync' written in x86-64 Linux assembly.
It flushes filesystem buffers by calling the sync syscall.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (sync: calls sync syscall, no output, exit 0)
"""
...
TIMEOUT = 10  # sync can take a moment on busy systems
BIN = ""
GNU = "/usr/bin/sync"
LOG_EVERY = 1
...
def find_binary():
    ...
    candidate = script_dir.parent / "fsync"
    if candidate.exists():
        BIN = str(candidate)
    else:
...
def check_syscall_surface():
    ...
    # sync should call: sync + exit_group
    cmd = ["strace", "-f", "-e",
           "trace=%process,%network,write,read,openat,open,creat,brk,mmap,mprotect,sync,fsync,fdatasync,syncfs",
           BIN]
    rc, out, err = run(cmd)
    ...
    # MUST call sync syscall
    sync_calls = [l for l in lines if "sync(" in l or "sync()" in l]
    # Note: strace may show "sync()" for the sync syscall
    has_sync = any("sync" in l and "(" in l for l in lines
                   if not l.startswith("execve") and "fsync" not in l and "syncfs" not in l
                   and "fdatasync" not in l)
    report_result(has_sync, "syscall: sync() called")
    ...
    # sync should NOT write to stdout
...
def check_fd_hygiene():
    ...
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    report_result(p.stdout.strip() == "0", "fd: closed stdout+stderr → exit 0")
    ...
    if os.path.exists("/dev/full"):
        ...
        p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
        report_result(p.stdout.strip() == "0", "fd: /dev/full → exit 0 (sync doesn't write)")
...
def check_tool_specific():
    ...
    rc, out, err = run([BIN])
    report_result(rc == 0, "sync: exit code 0")
    report_result(len(out) == 0, "sync: no stdout")
    report_result(len(err) == 0, "sync: no stderr")

    # Verify sync syscall is actually called (via strace)
    if which("strace"):
        cmd = ["strace", "-e", "trace=sync,fsync,fdatasync,syncfs", BIN]
        rc, out, err = run(cmd)
        err_text = err.decode(errors="replace")
        # Look for sync() call
        has_sync = "sync()" in err_text or "sync(" in err_text
        report_result(has_sync, "sync: actually calls sync() syscall")

    # sync ignores all arguments
    rc, out, err = run([BIN, "--help"])
    report_result(rc == 0, "sync: --help → exit 0 (ignores args)")

    # sync with file args tries to sync those files — nonexistent files give error
    rc, out, err = run([BIN, "some", "random", "args"])
    report_result(rc != 0, "sync: nonexistent file args → error exit")
    report_result(len(err) > 0, "sync: nonexistent file args → stderr message")

    # Compare with GNU sync
    if os.path.exists(GNU):
        rc_g, out_g, err_g = run([GNU])
        rc_f, out_f, err_f = run([BIN])
        report_result(rc_f == rc_g, "sync: exit code matches GNU")

    # Ignores stdin
    rc, out, err = run([BIN], stdin_data=b"data\n")
    report_result(rc == 0, "sync: ignores stdin → exit 0")
    report_result(len(out) == 0, "sync: ignores stdin → no stdout")

    # Multiple sync calls don't cause issues
    for i in range(10):
        rc, _, _ = run([BIN])
        if rc != 0:
            report_result(False, f"sync: repeated call {i} failed")
            break
    else:
        report_result(True, "sync: 10 repeated calls all exit 0")

    # With -- separator
    rc, _, _ = run([BIN, "--"])
    report_result(rc < 128, "sync: -- → no signal death")
