...
; Source files: tools/fdir.asm, lib/io.asm, lib/str.asm
;
; Build: nasm -f bin unified/fdir_unified.asm -o fdir_release && chmod +x fdir_release
;

BITS 64
org 0x400000

; --- System Constants (from linux.inc) ---
; linux.inc — Linux x86-64 syscall numbers and constants for fls
...
%define STAT_DEV            0       ; st_dev
%define STAT_INO            8       ; st_ino
%define STAT_NLINK         16       ; st_nlink
%define STAT_MODE          24       ; st_mode
%define STAT_UID           28       ; st_uid
%define STAT_GID           32       ; st_gid
%define STAT_RDEV          40       ; st_rdev
%define STAT_SIZE          48       ; st_size
%define STAT_BLKSIZE       56       ; st_blksize
%define STAT_BLOCKS        64       ; st_blocks
%define STAT_ATIME         72       ; st_atime
%define STAT_MTIME         88       ; st_mtime
%define STAT_CTIME        104       ; st_ctime
...
%define DIRENT_INO          0     ; d_ino (8 bytes)
%define DIRENT_OFF          8     ; d_off (8 bytes)
%define DIRENT_RECLEN      16     ; d_reclen (2 bytes)
%define DIRENT_TYPE        18     ; d_type (1 byte)
%define DIRENT_NAME        19     ; d_name (variable)
...
%define GETDENTS_SIZE    32768     ; 32KB for getdents64
%define PATH_MAX          4096
%define MAX_ENTRIES       8192     ; max dir entries we can sort

; --- Flag bits and mode defines ---
...
_start:
    ...
    movzx   eax, byte [invocation_mode]
    cmp     al, MODE_DIR
    je      .set_dir_default
    cmp     al, MODE_VDIR
    je      .set_vdir_default
...
.parse:
    ...
    test    word [flags], FLAG_L
    jz      .no_l_override
    and     word [flags], ~FLAG_MULTI_COL
.no_l_override:

    ; If -1 active, disable multi-column and long
    test    word [flags], FLAG_ONE
    jz      .no_1_override
    and     word [flags], ~(FLAG_MULTI_COL | FLAG_L)
.no_1_override:
...
.process_files:
    ...
    test    word [flags], FLAG_D
    jnz     .list_single_file

    ; lstat the argument
    push    rsi
    lea     rdx, [main_stat_buf]
    LSTAT   rsi, rdx
    pop     rsi
    test    rax, rax
    js      .stat_error
    ...
    mov     eax, [main_stat_buf + STAT_MODE]
    and     eax, S_IFMT
    cmp     eax, S_IFDIR
...
.list_single_file:
    ...
    ; If -i, print inode
    test    word [flags], FLAG_I
    jz      .single_no_inode
    mov     rax, [main_stat_buf + STAT_INO]
    call    emit_number
    mov     al, ' '
    call    emit_byte
.single_no_inode:
    ...
    ; If -s, print blocks
    test    word [flags], FLAG_S_BLOCKS
    jz      .single_no_blocks
    mov     rax, [main_stat_buf + STAT_BLOCKS]
...
list_directory:
    ...
    mov     qword [num_entries], 0

.ld_getdents_loop:
    mov     eax, SYS_GETDENTS64
    mov     edi, r15d
    lea     rsi, [getdents_buf]
...
.ld_accept_entry:
    ...
    lea     rdi, [entry_inodes]
    mov     [rdi + rax*8], rdx

    inc     qword [num_entries]
    pop     r14
    pop     rcx
...
;  We use [oe_loop_idx] for loop counters instead.
; ============================================================================
output_entries:
    push    rbx
    push    r13
    ...
    test    word [flags], FLAG_MULTI_COL
    jnz     .oe_multi_col

    ; One per line
    mov     qword [oe_loop_idx], 0
.oe_one_loop:
    mov     rax, [oe_loop_idx]
    cmp     rax, r15
    jge     .oe_done

    lea     rcx, [entry_sort_idx]
    mov     r13d, [rcx + rax*4]     ; actual index

    ; If -i, print inode
    test    word [flags], FLAG_I
    jz      .oe_no_inode
    lea     rax, [entry_inodes]
    mov     rdi, [rax + r13*8]
    mov     rax, rdi
    call    emit_number
    mov     al, ' '
...
.oe_mc_have_cols:
    mov     [mc_num_cols], eax
    mov     [mc_col_width], r14d

    mov     eax, r15d
    add     eax, [mc_num_cols]
    dec     eax
    xor     edx, edx
    div     dword [mc_num_cols]
    mov     [mc_num_rows], eax

    ; Output by rows, filling columns
    mov     dword [oe_row_idx], 0
.oe_mc_row:
    mov     eax, [oe_row_idx]
    cmp     eax, [mc_num_rows]
    jge     .oe_done
...
.oe_long_loop:
    ...
    test    word [flags], FLAG_I
    jz      .oe_long_no_inode
    mov     rax, [entry_stat_buf + STAT_INO]
    call    emit_number
    mov     al, ' '
    call    emit_byte
.oe_long_no_inode:
    ...
    mov     rax, [entry_stat_buf + STAT_BLOCKS]
    shr     rax, 1
    call    emit_number
    mov     al, ' '
    call    emit_byte
.oe_long_no_blocks:
    ...
    mov     eax, [entry_stat_buf + STAT_MODE]
    and     eax, S_IFMT
    cmp     eax, S_IFLNK
    ...
    lea     rcx, [readlink_buf]
    mov     byte [rcx + rax], 0
    mov     rdx, rax
    lea     rdi, [readlink_buf]
    call    emit_string_len
...
.pa_loop:
    ...
    ; Print "Try 'dir --help' for more information.\n"
    mov     rdi, STDERR
...
.pa_flag_l:
    ...
    jmp     .pa_short_next
.pa_flag_a:
    or      word [flags], FLAG_A
    jmp     .pa_short_next
.pa_flag_A:
    or      word [flags], FLAG_AA
    jmp     .pa_short_next
.pa_flag_1:
    or      word [flags], FLAG_ONE
    jmp     .pa_short_next
.pa_flag_R:
    or      word [flags], FLAG_R
    jmp     .pa_short_next
.pa_flag_r:
    or      word [flags], FLAG_REV
    jmp     .pa_short_next
.pa_flag_S:
    or      word [flags], FLAG_S_SORT
    jmp     .pa_short_next
.pa_flag_t:
    or      word [flags], FLAG_T_SORT
    jmp     .pa_short_next
.pa_flag_h:
    or      word [flags], FLAG_H
    jmp     .pa_short_next
.pa_flag_d:
    or      word [flags], FLAG_D
    jmp     .pa_short_next
.pa_flag_i:
    or      word [flags], FLAG_I
    jmp     .pa_short_next
.pa_flag_s:
    or      word [flags], FLAG_S_BLOCKS
    jmp     .pa_short_next
.pa_flag_C:
    or      word [flags], FLAG_MULTI_COL
    jmp     .pa_short_next
.pa_flag_g:
    or      word [flags], FLAG_L | FLAG_G
    jmp     .pa_short_next
...
.en_emit:
    cmp     ecx, 0
    jl      .en_done
    lea     rdi, [itoa_buf]     ; reload (emit_byte clobbers rdi)
    movzx   eax, byte [rdi + rcx]
    call    emit_byte
    dec     ecx
    jmp     .en_emit
...
; ============================================================================
;  Inlined from lib/io.asm
; ============================================================================

; io.asm — Shared I/O routines for fls
...
str_try_help:   db "Try 'dir --help' for more information.", 10
