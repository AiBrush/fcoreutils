; fdir.asm — GNU-compatible "ls" in x86_64 Linux assembly
;
; dir is identical to ls except default output is always multi-column (like ls -C).
;           --color=auto/always/never, --help, --version, --, multiple args
;
; This is derived from fls.asm with invocation_mode set to MODE_DIR.



;
; Build (modular):
;   nasm -f elf64 -I ./ tools/fdir.asm -o build/fdir.o
;   nasm -f elf64 -I ./ lib/io.asm -o build/io.o
;   nasm -f elf64 -I ./ lib/str.asm -o build/str.o
;   ld --gc-sections -z noexecstack build/fdir.o build/io.o build/str.o -o fdir
...
.pa_loop:
    ...
    mov     rdi, STDERR
    lea     rsi, [rel str_unrecognized_opt]
