...
finstall is a GNU-compatible 'install' written in x86-64 Linux assembly.
It copies files and sets attributes.
...
def find_binary():
    ...
    for name in ["finstall_release", "finstall"]:
...
def check_tool_specific():
    ...
    # --help
    rc, out, err = run([BIN, "--help"])
    report_result(rc == 0 and len(out) > 0, "install: --help works")
    ...
    try:
        ...
        with open(dst, "r") as f:
            content = f.read()
        report_result(content == "test content\n", "install: basic file copy")

        # Check permissions (should be 755)
        mode = oct(os.stat(dst).st_mode)[-3:]
        report_result(mode == "755", f"install: default mode 755 (got {mode})")
    finally:
        os.unlink(src)
        os.unlink(dst)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        rc, out, err = run([BIN, "-d", new_dir])
        report_result(os.path.isdir(new_dir), "install: -d creates directory")

    # Missing operand
    rc, out, err = run([BIN])
    report_result(rc != 0, "install: missing operand exits non-zero")
