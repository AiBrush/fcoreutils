#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fb2sum (assembly b2sum).

fb2sum is a GNU-compatible 'b2sum' written in x86-64 Linux assembly.
It computes BLAKE2b checksums.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (b2sum: BLAKE2b checksum behavior)
"""
...
BSS_SIZE = 65536
TOOL_NAME = "b2sum"
BIN = str(Path(__file__).resolve().parent.parent / "fb2sum")
GNU = "/usr/bin/b2sum"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "fb2sum", "proc: /proc/PID/exe points to fb2sum")
        except Exception as e:
...
# =============================================================================
#                     13. TOOL-SPECIFIC: B2SUM TESTS
# =============================================================================

def test_b2sum_specific():
    ...
    for data, expected_hash in vectors:
        ...
        if rc == 0:
            ...
            report_result(False, f"b2sum: vector failed (rc={rc})")
    ...
    for data, expected_hash in vectors:
        ...
        if rc_a == 0 and rc_g == 0:
            ...
            report_result(hash_a == hash_g, f"b2sum: GNU match '{data[:20].decode(errors='replace')}'")
    ...
    if rc == 0:
        ...
        report_result(False, "b2sum: output format check (command failed)")

    # Compare with GNU output format
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "b2sum: output format matches GNU exactly")
    ...
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "b2sum: all 256 byte values hash correct")
    ...
    if rc == 0:
        output_hash = out.decode().strip().split()[0] if out else ""
        report_result(output_hash == expected, "b2sum: 1MB hash correct")
    else:
        report_result(False, "b2sum: 1MB hash (command failed)")
    ...
    for i in range(20):
        ...
        if rc == 0:
            output_hash = out.decode().strip().split()[0] if out else ""
            report_result(output_hash == expected, f"b2sum: random {size} bytes hash correct")
        else:
            report_result(False, f"b2sum: random {size} bytes (command failed)")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        report_result(out_a == out_g and rc_a == rc_g, "b2sum: file hash matches GNU")

    # --help/--version
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "b2sum: --help works")

    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "b2sum: --version works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (fb2sum) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
