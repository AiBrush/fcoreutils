#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fwho."""
...
def test_help():
    ...
    text = out + err
    report_result(b"Usage" in text or b"who" in text, "--help output")


def test_version():
    rc, out, err = run([BIN, "--version"])
    text = out + err
    report_result(b"coreutils" in text or b"who" in text, "--version output")
