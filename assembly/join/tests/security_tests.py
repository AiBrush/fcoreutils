#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fjoin (assembly join)."""
...
TIMEOUT = 5
TOOL_NAME = "join"
BIN = str(Path(__file__).resolve().parent.parent / "fjoin")
GNU = "/usr/bin/join"
...
def test_syscall_surface():
    ...
    f1 = make_temp_file(b"a 1\nb 2\nc 3\n")
    f2 = make_temp_file(b"b x\nc y\nd z\n")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                report_result(os.path.basename(exe) == "fjoin", "proc: /proc/PID/exe points to fjoin")
            except Exception as e:
...
def test_memory_safety():
    ...
    try:
        ...
        for desc, data1, data2 in [
            ("empty files", b"", b""),
            ("empty + non-empty", b"", b"a 1\nb 2\n"),
            ("single byte each", b"A\n", b"B\n"),
            ("identical files", b"a 1\nb 2\nc 3\n", b"a 1\nb 2\nc 3\n"),
            ("binary data", bytes(range(1, 256)) + b"\n", bytes(range(1, 256)) + b"\n"),
            ("null bytes in lines", b"he\x00llo 1\nworld 2\n", b"he\x00llo 1\ntest 2\n"),
        ]:
        ...
        # Large files
        log("\n--- Large File Tests ---")
        large_data1 = b"".join(f"line_{i:010d} val1\n".encode() for i in range(100000))
        large_data2 = b"".join(f"line_{i:010d} val2\n".encode() for i in range(50000, 150000))
        ...
        log("\n--- Boundary Value Analysis ---")
        for desc, data1, data2 in [
            ("no trailing newline", b"hello 1", b"hello 2"),
            ("CRLF endings", b"line1 1\r\nline2 2\r\n", b"line1 x\r\nline3 y\r\n"),
            ("only newlines", b"\n\n\n", b"\n\n"),
        ...
        report_result(rc < 128, "mem: RLIMIT_AS=256MB")
...
def test_signal_safety():
    ...
    f1 = make_temp_file(b"".join(f"line_{i:06d} val\n".encode() for i in range(10000)))
    f2 = make_temp_file(b"".join(f"line_{i:06d} dat\n".encode() for i in range(5000, 15000)))
    ...
    try:
        ...
        for sig_val, sig_name in [(signal.SIGTERM, "SIGTERM"), (signal.SIGINT, "SIGINT")]:
            f1b = make_temp_file(b"".join(f"line_{i:06d} v\n".encode() for i in range(1000000)))
            f2b = make_temp_file(b"".join(f"line_{i:06d} v\n".encode() for i in range(500000, 1500000)))
            try:
...
def test_correctness():
    ...
    temp_files = []
    try:
        # Compare against GNU join for various inputs
        test_cases = [
            ("basic", b"1 a\n2 b\n3 c\n", b"1 x\n2 y\n3 z\n", []),
            ("identical", b"a 1\nb 2\nc 3\n", b"a 1\nb 2\nc 3\n", []),
            ("disjoint", b"x 1\ny 2\n", b"a 3\nb 4\n", []),
