; fjoin.asm — GNU-compatible "join" in x86_64 Linux assembly
;
; Sorted file merge with field extraction. For each pair of lines from
; two sorted files that have matching join fields, output a line containing
; the join field plus remaining fields from both files.
;
; Supported flags:
;   -t CHAR           field separator (default: whitespace)
;   -1 FIELD          join field of file 1 (1-indexed)
;   -2 FIELD          join field of file 2 (1-indexed)
;   -j FIELD          same join field in both files
...
;   - SSE2 SIMD line/field scanning
;   - 1MB output buffer with threshold flushing
;   - For stdin (-), read into 16MB buffer
;
; Build (modular):
;   nasm -f elf64 -I include/ tools/fjoin.asm -o build/fjoin.o
;   nasm -f elf64 -I include/ lib/io.asm -o build/io.o
;   ld --gc-sections build/fjoin.o build/io.o -o fjoin
...
%define MAX_EMPTY_LEN   256             ; max -e string length
%define MAX_FMT_SPECS   64              ; max -o format specs
%define MAX_LINE_PTRS   4194304         ; max lines per file (4M)
...
opt_sep_set:            resb 1
opt_sep_char:           resb 1
opt_ignore_case:        resb 1
opt_zero_term:          resb 1
opt_header:             resb 1
opt_order_check:        resb 1
opt_print_unpaired1:    resb 1
opt_print_unpaired2:    resb 1
opt_only_unpaired1:     resb 1
opt_only_unpaired2:     resb 1
opt_auto_format:        resb 1
seen_dashdash:          resb 1

alignb 8
opt_empty_len:          resq 1
opt_empty_buf:          resb MAX_EMPTY_LEN

alignb 8
opt_fmt_count:          resq 1
fmt_spec_types:         resb MAX_FMT_SPECS
fmt_spec_file:          resb MAX_FMT_SPECS
alignb 8
fmt_spec_field:         resq MAX_FMT_SPECS

; Computed flags
print_paired:           resb 1
show_unpaired1:         resb 1
show_unpaired2:         resb 1
out_sep:                resb 1
line_delim:             resb 1
...
alignb 8
lines1_ptrs:            resq MAX_LINE_PTRS
lines1_lens:            resq MAX_LINE_PTRS
lines2_ptrs:            resq MAX_LINE_PTRS
lines2_lens:            resq MAX_LINE_PTRS

; Pre-computed join key arrays
alignb 8
keys1_ptrs:             resq MAX_LINE_PTRS
keys1_lens:             resq MAX_LINE_PTRS
keys2_ptrs:             resq MAX_LINE_PTRS
keys2_lens:             resq MAX_LINE_PTRS
...
cur_key2_len:           resq 1
group2_start:           resq 1
group_key_ptr:          resq 1
group_key_len:          resq 1
cross_j:                resq 1

; Temporary storage
tmp_key_ptr:            resq 1
tmp_key_len:            resq 1
tmp_prev_key_ptr:       resq 1
tmp_prev_key_len:       resq 1
