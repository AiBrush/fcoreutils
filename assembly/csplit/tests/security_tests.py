...
fcsplit is a GNU-compatible 'csplit' written in x86-64 Linux assembly.
It splits files based on context.
...
def find_binary():
    ...
    for name in ["fcsplit_release", "fcsplit"]:
...
def check_tool_specific():
    ...
    # --help exits 0
    rc, out, err = run([BIN, "--help"])
    report_result(rc == 0 and len(out) > 0, "csplit: --help works")

    # Missing operand
    rc, out, err = run([BIN])
    report_result(rc != 0, "csplit: missing operand exits non-zero")
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            import subprocess
            p = subprocess.run([BIN, "-f", os.path.join(td, "out"), tmp, "5"],
                             capture_output=True, timeout=5)
            files = sorted([f for f in os.listdir(td) if f.startswith("out")])
            report_result(len(files) >= 1, f"csplit: created output files ({len(files)})")
    finally:
        os.unlink(tmp)
