; ============================================================
; flogname_unified.asm — GNU-compatible 'logname' command
; Builds with: nasm -f bin flogname_unified.asm -o flogname
; ============================================================
...
%define OFF_READ_BUF    0
%define OFF_NAME_BUF    4096
%define OFF_UID_STR     4352
%define OFF_TTY_BUF     4384
%define OFF_UTMP_ENTRY  4640
...
try_loginuid:
    ...
    jle     .tl_fail
    mov     r12, rax
    mov     byte [BSS_ADDR + OFF_UID_STR + r12], 0
    dec     r12
    cmp     byte [BSS_ADDR + OFF_UID_STR + r12], 10
    jne     .tl_no_trim
    mov     byte [BSS_ADDR + OFF_UID_STR + r12], 0
.tl_no_trim:
    mov     rdi, BSS_ADDR + OFF_UID_STR
...
.sp_read:
    mov     rdi, r12
    lea     rsi, [BSS_ADDR + OFF_READ_BUF + rbp]
    mov     rdx, 4095
    sub     rdx, rbp
    jle     .sp_nf
    call    do_read
...
.sp_nl:
    cmp     r14, rbp
    jge     .sp_partial
    cmp     byte [BSS_ADDR + OFF_READ_BUF + r14], 10
    je      .sp_line
    inc     r14
    jmp     .sp_nl
.sp_line:
...
.sp_fc:
    cmp     rcx, r14
    jge     .sp_next
    cmp     byte [BSS_ADDR + OFF_READ_BUF + rcx], ':'
    jne     .sp_fc2
    inc     rbx
    cmp     rbx, 2
    je      .sp_us
...
.sp_cb:
    cmp     rbx, rax
    je      .sp_match
    movzx   edx, byte [BSS_ADDR + OFF_READ_BUF + r15 + rbx]
    cmp     dl, byte [BSS_ADDR + OFF_UID_STR + rbx]
    jne     .sp_next
    inc     rbx
    jmp     .sp_cb
.sp_match:
    lea     rsi, [BSS_ADDR + OFF_READ_BUF + r13]
    mov     rdi, BSS_ADDR + OFF_NAME_BUF
    xor     rcx, rcx
.sp_cn:
    cmp     byte [rsi + rcx], ':'
    je      .sp_cd
...
try_utmp:
    ...
    mov     rdi, path_fd0
    mov     rsi, BSS_ADDR + OFF_TTY_BUF
    mov     rdx, 255
    syscall
    test    rax, rax
    jle     .tu_fail
    mov     byte [BSS_ADDR + OFF_TTY_BUF + rax], 0
    cmp     byte [BSS_ADDR + OFF_TTY_BUF], '/'
    jne     .tu_fail
    cmp     byte [BSS_ADDR + OFF_TTY_BUF + 1], 'd'
    jne     .tu_fail
    cmp     byte [BSS_ADDR + OFF_TTY_BUF + 2], 'e'
    jne     .tu_fail
    cmp     byte [BSS_ADDR + OFF_TTY_BUF + 3], 'v'
    jne     .tu_fail
    cmp     byte [BSS_ADDR + OFF_TTY_BUF + 4], '/'
    jne     .tu_fail
    lea     r13, [BSS_ADDR + OFF_TTY_BUF + 5]
    mov     rax, SYS_OPEN
    mov     rdi, path_utmp
    xor     rsi, rsi
    xor     rdx, rdx
...
.tu_read:
    mov     rdi, r12
    mov     rsi, BSS_ADDR + OFF_UTMP_ENTRY
    mov     rdx, UTMP_ENTRY_SIZE
    call    do_read
    cmp     rax, UTMP_ENTRY_SIZE
    jne     .tu_close
    movzx   eax, word [BSS_ADDR + OFF_UTMP_ENTRY + UTMP_TYPE_OFF]
    cmp     eax, USER_PROCESS
    jne     .tu_read
    lea     rdi, [BSS_ADDR + OFF_UTMP_ENTRY + UTMP_LINE_OFF]
    mov     rsi, r13
    call    str_eq_func
    test    rax, rax
    jz      .tu_read
    lea     rsi, [BSS_ADDR + OFF_UTMP_ENTRY + UTMP_USER_OFF]
    mov     rdi, BSS_ADDR + OFF_NAME_BUF
    xor     rcx, rcx
.tu_cn:
...
str_help:
    ...
    db "Print the user's login name.", 10, 10
...
str_version:
    db "logname (GNU coreutils) 9.7", 10
