#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fstat.

fstat is a GNU-compatible 'stat' written in x86-64 Linux assembly.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (stat: file status display)
"""
...
def find_binary():
    ...
    for name in ["fstat_release", "fstat"]:
...
def setup_fixtures():
    global TMPDIR
    TMPDIR = tempfile.mkdtemp(prefix="fstat_test_")
    Path(f"{TMPDIR}/testfile").touch()
    os.makedirs(f"{TMPDIR}/testdir", exist_ok=True)
    os.symlink(f"{TMPDIR}/testfile", f"{TMPDIR}/symlink")
...
def check_syscall_surface():
    ...
    if not which("strace"):
        ...
        return
    cmd = ["strace", "-f", "-e", "trace=%network", BIN, f"{TMPDIR}/testfile"]
    rc, out, err = run(cmd)
...
def check_proc_analysis():
    ...
    rc, out, err = run([BIN, f"{TMPDIR}/testfile"])
    report_result(rc == 0, "proc: tool runs cleanly")


def check_fd_hygiene():
    log("\n=== 4. File Descriptor Hygiene ===")
    script = f'exec 3>&1 1>&-; {BIN} {TMPDIR}/testfile 2>/dev/null; echo $? >&3'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    rc = p.stdout.strip()
    report_result(rc != "", "fd: closed stdout does not hang")

    if os.path.exists("/dev/null"):
        script = f'{BIN} {TMPDIR}/testfile > /dev/null 2>/dev/null; echo $?'
        p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
        rc = p.stdout.strip()
...
def check_memory_safety():
    ...
    rc, out, err = run([BIN, f"{TMPDIR}/testfile"], preexec_fn=limit_stack)
    report_result(rc >= 0 and rc < 128, "memory: 64KB stack no crash")


def check_signal_safety():
    log("\n=== 6. Signal Safety ===")
    script = f'{BIN} {TMPDIR}/testfile | head -c 0'
...
def check_output_integrity():
    ...
    outputs = []
    for _ in range(5):
        rc, out, err = run([BIN, "-c", "%i", f"{TMPDIR}/testfile"])
...
def check_error_handling():
    ...
    report_result(rc == 1, "error: nonexistent file exits 1")
...
def check_concurrency():
    ...
    procs = []
    for _ in range(30):
        p = subprocess.Popen([BIN, f"{TMPDIR}/testfile"],
...
def check_tool_specific():
    log("\n=== 13. Tool-Specific: stat ===")
    gnu_path = which(GNU)

    # Basic format specifiers
    for spec in ["%n", "%s", "%b", "%i", "%h", "%u", "%g", "%a", "%o",
                 "%X", "%Y", "%Z", "%W", "%F"]:
        rc, out, _ = run([BIN, "-c", spec, f"{TMPDIR}/testfile"])
    ...
    if gnu_path:
        for spec in ["%n", "%s", "%b", "%i", "%h", "%u", "%g", "%a",
                     "%X", "%Y", "%Z", "%W"]:
            rc_f, out_f, _ = run([BIN, "-c", spec, f"{TMPDIR}/testfile"])
            rc_g, out_g, _ = run([gnu_path, "-c", spec, f"{TMPDIR}/testfile"])
            report_result(out_f == out_g,
                         f"stat: -c {spec} matches GNU")

    # File type detection
    rc, out, _ = run([BIN, "-c", "%F", f"{TMPDIR}/testfile"])
    report_result(b"regular" in out, "stat: detects regular file")

    rc, out, _ = run([BIN, "-c", "%F", f"{TMPDIR}/testdir"])
    report_result(b"directory" in out, "stat: detects directory")

    rc, out, _ = run([BIN, "-c", "%F", f"{TMPDIR}/symlink"])
    report_result(b"symbolic link" in out, "stat: detects symlink (no -L)")

    # -L follows symlinks
    rc, out, _ = run([BIN, "-L", "-c", "%F", f"{TMPDIR}/symlink"])
    report_result(b"regular" in out, "stat: -L follows symlink")

    # Terse output
    rc, out, _ = run([BIN, "-t", f"{TMPDIR}/testfile"])
    report_result(rc == 0 and len(out.split()) >= 10,
                 "stat: -t terse output has many fields")

    # Error on nonexistent
    rc, out, err = run([BIN, f"{TMPDIR}/nonexistent"])
    report_result(rc == 1, "stat: nonexistent exits 1")
    report_result(b"cannot stat" in err or b"No such" in err,
