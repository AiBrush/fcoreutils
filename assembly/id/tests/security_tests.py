...
fid is a GNU-compatible 'id' written in x86-64 Linux assembly.
It prints real and effective user and group IDs.
...
def check_output_integrity():
    ...
    if gnu_path:
        # Compare -u, -g, -un, -gn
        for args in [["-u"], ["-g"], ["-un"], ["-gn"]]:
            rc_f, out_f, _ = run([BIN] + args)
            rc_g, out_g, _ = run([gnu_path] + args)
            report_result(rc_f == rc_g and out_f == out_g,
                          f"output: {' '.join(args)} matches GNU")
...
def check_tool_specific():
    ...
    report_result(rc == 0, "id: no args -> exit 0")
    report_result("uid=" in output, "id: output contains uid=")
    report_result("gid=" in output, "id: output contains gid=")
    report_result("groups=" in output, "id: output contains groups=")

    # -u
    rc, out, _ = run([BIN, "-u"])
    uid_str = out.decode().strip()
    euid = os.geteuid()
    report_result(rc == 0 and uid_str == str(euid),
    ...
    egid = os.getegid()
    report_result(rc == 0 and gid_str == str(egid),
    ...
    report_result(rc == 0 and uname == expected_name,
    ...
    report_result(rc == 0 and gname == expected_gname,
                  f"id -gn: '{gname}' matches '{expected_gname}'")

    # -G
    rc, out, _ = run([BIN, "-G"])
    report_result(rc == 0, "id -G: exit 0")
    gids = out.decode().strip().split()
    report_result(len(gids) >= 1, f"id -G: at least 1 group ({len(gids)} found)")

    # -Gn
    rc, out, _ = run([BIN, "-Gn"])
    report_result(rc == 0, "id -Gn: exit 0")
    names = out.decode().strip().split()
    report_result(len(names) >= 1, f"id -Gn: at least 1 group name ({len(names)} found)")
    ...
    if gnu_path:
        ...
        # -G comparison (as sets since order may differ for supplementary)
        rc_f, out_f, _ = run([BIN, "-G"])
        rc_g, out_g, _ = run([gnu_path, "-G"])
        f_gids = set(out_f.decode().strip().split())
        g_gids = set(out_g.decode().strip().split())
        report_result(f_gids == g_gids, "id -G: group ID sets match GNU")

    # With username
    username = pwd.getpwuid(os.getuid()).pw_name
    rc, out, _ = run([BIN, username])
    report_result(rc == 0, f"id '{username}': exit 0")

    # Nonexistent user
    rc, _, err = run([BIN, "nonexistent_user_xyz"])
    report_result(rc == 1, "id nonexistent: exit 1")
    report_result(len(err) > 0, "id nonexistent: stderr output")

    # --help
    rc, out, _ = run([BIN, "--help"])
    report_result(rc == 0, "id --help: exit 0")
    report_result(b"Usage:" in out, "id --help: contains Usage:")

    # --version
    rc, out, _ = run([BIN, "--version"])
    report_result(rc == 0, "id --version: exit 0")
    report_result(b"id" in out, "id --version: contains 'id'")

    # Multiple runs same result
    results = [run([BIN])[1] for _ in range(10)]
    report_result(all(r == results[0] for r in results), "id: 10 runs same output")
