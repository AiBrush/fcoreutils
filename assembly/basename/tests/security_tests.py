...
fbasename is a GNU-compatible 'basename' written in x86-64 Linux assembly.
It strips directory and suffix from filenames.
...
def find_binary():
    ...
    for name in ["fbasename_release", "fbasename"]:
...
def check_syscall_surface():
    ...
    # basename should only use write + exit + rt_sigprocmask
...
def check_fd_hygiene():
    ...
    rc = p.stdout.strip()
    # basename writes to stdout; closed stdout may cause exit != 0, but must not hang/crash
...
def check_memory_safety():
    ...
    rc, out, err = run([BIN, "-a"] + [f"/path/to/file{i}" for i in range(1000)])
...
def check_output_integrity():
    ...
    report_result(all_correct, "output: all 10 runs produce 'sort\\n'")
...
def check_tool_specific():
    ...
    for args, expected in test_cases:
        rc, out, err = run([BIN] + args)
        report_result(out == expected, f"basename: {' '.join(args)} → {expected.rstrip().decode()}")

    # -z produces actual NUL byte terminator
    rc, out, err = run([BIN, "-z", "/usr/bin/sort"])
    report_result(out == b"sort\x00", "basename: -z produces NUL byte")
    report_result(b"\n" not in out, "basename: -z no trailing newline")

    # Extremely long path
    long_path = "/" + "/".join(["a" * 200] * 10)
    rc, out, err = run([BIN, long_path])
    report_result(rc == 0 and out == b"a" * 200 + b"\n",
    ...
    if gnu_path:
        ...
        report_result(out == out_g, "basename: path with embedded newlines matches GNU")

    # Multiple consecutive slashes
    rc, out, err = run([BIN, "///usr///bin///sort"])
    report_result(out == b"sort\n", "basename: multiple consecutive slashes handled")

    # Suffix edge cases
    rc, out, err = run([BIN, "file.txt", ".txt"])
    report_result(out == b"file\n", "basename: suffix removal .txt")

    rc, out, err = run([BIN, "file.txt", ".c"])
    report_result(out == b"file.txt\n", "basename: suffix no match .c")

    rc, out, err = run([BIN, ".txt", ".txt"])
    report_result(out == b".txt\n", "basename: suffix same as name → no removal")

    # --suffix= form
    rc, out, err = run([BIN, "--suffix=.h", "stdio.h"])
    report_result(out == b"stdio\n", "basename: --suffix=.h works")

    # Combined flags
    rc, out, err = run([BIN, "-az", ".h", "stdio.h", "errno.h"])
    report_result(b"\x00" in out, "basename: -az combined produces NUL")
