#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for fpwd.

fpwd is a GNU-compatible 'pwd' written in x86-64 Linux assembly.
It prints the current working directory. Uses getcwd syscall.
...
TEST CATEGORIES:
   ...
   13. Tool-specific (pwd behavior: matches $PWD, deep paths, symlinks)
"""
...
def find_binary():
    ...
    candidate = script_dir.parent / "fpwd"
    if candidate.exists():
        BIN = str(candidate)
    else:
...
def check_syscall_surface():
    ...
    cmd = ["strace", "-f", "-e",
           "trace=%process,%network,write,read,openat,open,creat,brk,mmap,mprotect,getcwd",
           BIN]
    rc, out, err = run(cmd)
    ...
    # Should have getcwd
    getcwd_calls = [l for l in lines if "getcwd(" in l]
    report_result(len(getcwd_calls) >= 1, "syscall: getcwd() called")
...
def check_environment():
    ...
    # PWD set to wrong value — pwd should use getcwd, not $PWD
    env = os.environ.copy()
    env["PWD"] = "/fake/directory/that/doesnt/exist"
    rc, out, _ = run([BIN], env=env)
    if rc == 0:
        pwd_out = out.decode().strip()
        report_result(pwd_out != "/fake/directory/that/doesnt/exist",
                      "env: ignores fake $PWD, uses getcwd")
...
def check_output_integrity():
    ...
    report_result(len(err) == 0, "output: no stderr")
    ...
    if os.path.exists(GNU):
        ...
        report_result(out_f == out_g, "output: stdout matches GNU pwd")
...
def check_concurrency():
    ...
    # All should output the same directory
    outs = []
    for _ in range(20):
        rc, out, _ = run([BIN])
        if rc == 0:
            outs.append(out)
    if outs:
        report_result(all(o == outs[0] for o in outs), "concurrency: 20 runs same output")
...
def check_tool_specific():
    ...
    report_result(rc == 0, "pwd: exit code 0")
    report_result(len(pwd_str) > 0, f"pwd: non-empty output")
    report_result(out.endswith(b"\n"), "pwd: output ends with newline")
    report_result(len(err) == 0, "pwd: no stderr output")

    # Must match os.getcwd()
    cwd = os.getcwd()
    report_result(pwd_str == cwd, f"pwd: '{pwd_str}' matches os.getcwd() '{cwd}'")

    # Exactly one line
    lines = out.decode(errors="replace").split("\n")
    non_empty = [l for l in lines if l]
    report_result(len(non_empty) == 1, "pwd: exactly one line of output")

    # Output is an absolute path
    report_result(pwd_str.startswith("/"), "pwd: output is absolute path")

    # Compare with GNU pwd
    if os.path.exists(GNU):
        rc_g, out_g, _ = run([GNU])
        report_result(out == out_g, "pwd: output matches GNU pwd")

    # Works from root directory
    rc, out, _ = run([BIN], cwd="/")
    if rc == 0:
        report_result(out.strip() == b"/", "pwd: from / → outputs /")

    # Works from /tmp
    rc, out, _ = run([BIN], cwd="/tmp")
    if rc == 0:
        # /tmp might be a symlink, so resolve both
        actual = out.decode().strip()
        expected = os.path.realpath("/tmp")
        report_result(actual == "/tmp" or actual == expected,
                      f"pwd: from /tmp → '{actual}'")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        rc, out, _ = run([BIN], cwd=deep)
        if rc == 0:
            report_result(out.decode().strip() == deep,
                          f"pwd: deep path (20 levels) correct")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        rc, out, _ = run([BIN], cwd=link_dir)
        if rc == 0:
            actual = out.decode().strip()
            # pwd may resolve symlinks or not — both are valid
            report_result(actual == link_dir or actual == real_dir,
                          f"pwd: symlink dir → '{actual}'")

    # Ignores arguments
    rc, out, _ = run([BIN, "ignored"])
    report_result(rc < 128, "pwd: with extra arg → no signal death")

    # Multiple runs same result
    results = [run([BIN])[1] for _ in range(10)]
    report_result(all(r == results[0] for r in results), "pwd: 10 runs same output")

    # -P flag (physical, resolve symlinks)
    if os.path.exists(GNU):
        rc_f, out_f, _ = run([BIN, "-P"])
        rc_g, out_g, _ = run([GNU, "-P"])
        if rc_f == 0 and rc_g == 0:
            report_result(out_f == out_g, "pwd: -P output matches GNU")

    # With -- separator
    rc, _, _ = run([BIN, "--"])
    report_result(rc < 128, "pwd: -- → no signal death")
