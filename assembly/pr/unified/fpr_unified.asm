...
; Source files: tools/fpr.asm, lib/io.asm
;
; Build: nasm -f bin unified/fpr_unified.asm -o fpr_release && chmod +x fpr_release
;
; GNU-compatible "pr" in x86-64 Linux assembly.
; mmap input, 512KB output buffer, SWAR newline scanning, TZ/date caching.
; Supports: +PAGE, -COLUMN, -a, -c, -d, -f/-F, -h, -i, -J, -l, -m, -n,
;           -N, -o, -r, -s, -S, -t, -T, -v, -w, -W, -e, -D, --help, --version
...
parse_long_option:
    ...
    mov     rsi, r14
    lea     rdi, [str_double_space_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_double_space

    mov     rsi, r14
    lea     rdi, [str_form_feed_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_form_feed

    mov     rsi, r14
    lea     rdi, [str_join_lines_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_join_lines

    mov     rsi, r14
    lea     rdi, [str_merge_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_merge

    mov     rsi, r14
    lea     rdi, [str_number_lines_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_number_lines

    mov     rsi, r14
    lea     rdi, [str_no_file_warn_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_no_file_warn

    mov     rsi, r14
    lea     rdi, [str_omit_header_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_omit_header

    mov     rsi, r14
    lea     rdi, [str_omit_pagination_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_omit_pagination

    mov     rsi, r14
    lea     rdi, [str_show_ctrl_flag]
    call    strcmp
    test    eax, eax
    jz      .plo_show_ctrl

    mov     rsi, r14
    lea     rdi, [str_show_nonprint_flag]
    call    strcmp
    test    eax, eax
...
.psf_stdin:
    ...
    ; Process data
    lea     rdi, [str_empty]    ; empty header for stdin
...
.ppftr_formfeed:
    ...
    mov     byte [single_char_buf], 12   ; FF
    lea     rsi, [single_char_buf]
    mov     edx, 1
    call    buffer_output
...
.obl_no_indent:
    ...
    lea     rsi, [num_format_buf]
    mov     rdx, rax
    call    buffer_output

    ; Output separator
    movzx   eax, byte [opt_number_sep]
    mov     [single_char_buf], al
    lea     rsi, [single_char_buf]
    mov     edx, 1
    call    buffer_output
...
; ═══════════════════════════════════════════════════════════
; ═══════════════════════════════════════════════════════════
; I/O Library Functions (from lib/io.asm)
; ═══════════════════════════════════════════════════════════
