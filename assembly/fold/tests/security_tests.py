...
BSS_SIZE = 65536
TOOL_NAME = "fold"
BIN = str(Path(__file__).resolve().parent.parent / "ffold")
GNU = "/usr/bin/fold"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "ffold", "proc: /proc/PID/exe points to ffold")
        except Exception as e:
...
def test_memory_safety():
    ...
    for desc, size in [
        ...
        # Single line of that size (fold wraps long lines, so long lines matter most)
        data = b"A" * size + b"\n"
        rc, _, _ = run_asm([], stdin_data=data)
...
def test_input_fuzzing():
    ...
    test_data = b"hello world this is a test line\n"
    results = set()
    for _ in range(10):
...
def test_concurrency():
    ...
    procs = []
    for i in range(50):
        data = f"instance {i} line of text for folding\n".encode() * 10
        p = subprocess.Popen([BIN], stdin=subprocess.PIPE,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        procs.append((p, data))
    ...
    # fold | fold idempotency (folding already-wrapped text should be no-op)
    short_data = b"short\n"
    rc1, out1, _ = run_asm([], stdin_data=short_data)
    rc2, out2, _ = run_asm([], stdin_data=out1)
    report_result(out1 == out2, "concurrency: pipe chain fold|fold idempotent for short lines")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: FOLD TESTS
# =============================================================================

def test_fold_specific():
    log("\n=== Fold-Specific Tests ===")

    # Default 80-char wrap
    line80 = b"A" * 80 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=line80)
    rc_g, out_g, _ = run_gnu([], stdin_data=line80)
    report_result(out_a == out_g, "fold: 80-char line (at limit) matches GNU")

    line81 = b"A" * 81 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=line81)
    rc_g, out_g, _ = run_gnu([], stdin_data=line81)
    report_result(out_a == out_g, "fold: 81-char line (over limit) matches GNU")

    line79 = b"A" * 79 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=line79)
    rc_g, out_g, _ = run_gnu([], stdin_data=line79)
    report_result(out_a == out_g, "fold: 79-char line (under limit) matches GNU")

    # -w N (custom width)
    for width in [10, 20, 40, 1, 2]:
        data = b"A" * 100 + b"\n"
        rc_a, out_a, _ = run_asm(["-w", str(width)], stdin_data=data)
        rc_g, out_g, _ = run_gnu(["-w", str(width)], stdin_data=data)
        report_result(out_a == out_g, f"fold: -w {width} matches GNU")

    # -s (break at spaces)
    data = b"the quick brown fox jumps over the lazy dog and keeps on going forever\n"
    rc_a, out_a, _ = run_asm(["-s", "-w", "20"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", "-w", "20"], stdin_data=data)
    report_result(out_a == out_g, "fold: -s -w 20 (break at spaces) matches GNU")

    # -s with no spaces (should still wrap)
    data = b"A" * 100 + b"\n"
    rc_a, out_a, _ = run_asm(["-s", "-w", "20"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", "-w", "20"], stdin_data=data)
    report_result(out_a == out_g, "fold: -s with no spaces matches GNU")

    # -b (count bytes, not columns)
    data = b"hello\tworld\n"
    rc_a, out_a, _ = run_asm(["-b", "-w", "8"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "-w", "8"], stdin_data=data)
    report_result(out_a == out_g, "fold: -b -w 8 (count bytes) matches GNU")

    # -b -s combined
    data = b"the quick brown fox jumps over the lazy dog\n"
    rc_a, out_a, _ = run_asm(["-b", "-s", "-w", "15"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "-s", "-w", "15"], stdin_data=data)
    report_result(out_a == out_g, "fold: -b -s -w 15 matches GNU")

    # Empty input
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "fold: empty input matches GNU")

    # Single newline
    rc_a, out_a, _ = run_asm([], stdin_data=b"\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"\n")
    report_result(out_a == out_g, "fold: single newline matches GNU")

    # Multiple short lines (no wrapping needed)
    data = b"short\nlines\nhere\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "fold: short lines (no wrap) matches GNU")

    # Tabs (default tab expansion to 8)
    data = b"\thello\t\tworld\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "fold: tabs matches GNU")

    # Long line with spaces, -s
    data = b" ".join([b"word"] * 50) + b"\n"
    rc_a, out_a, _ = run_asm(["-s"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s"], stdin_data=data)
    report_result(out_a == out_g, "fold: -s long sentence matches GNU")

    # Multiple lines, some long, some short
    data = b"short\n" + b"A" * 200 + b"\nmedium length line here\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "fold: mixed line lengths matches GNU")

    # No trailing newline
    data = b"hello world"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "fold: no trailing newline matches GNU")
    ...
    try:
        ...
        report_result(out_a == out_g, "fold: file argument matches GNU")
    finally:
        os.unlink(tmpfile)

    # Stdin explicitly via -
    data = b"fold this line please\n"
    rc_a, out_a, _ = run_asm(["-w", "10", "-"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w", "10", "-"], stdin_data=data)
    report_result(out_a == out_g, "fold: stdin via '-' matches GNU")

    # Very long line (10KB)
    data = b"X" * 10000 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "fold: very long line (10KB) matches GNU")

    # Large input (10K lines)
    large = b"".join(f"line{i:06d} with some extra text to fill up space\n".encode() for i in range(10000))
    rc_a, out_a, _ = run_asm([], stdin_data=large, timeout=10)
    rc_g, out_g, _ = run_gnu([], stdin_data=large, timeout=10)
    report_result(out_a == out_g, "fold: large input (10K lines) matches GNU")

    # --help
    rc_a, out_a, _ = run_asm(["--help"])
    report_result(rc_a == 0 and len(out_a) > 0, "fold: --help works")

    # --version
    rc_a, out_a, _ = run_asm(["--version"])
    report_result(rc_a == 0 and len(out_a) > 0, "fold: --version works")

    # -w 1 (wrap every character)
    data = b"hello\n"
    rc_a, out_a, _ = run_asm(["-w", "1"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-w", "1"], stdin_data=data)
    report_result(out_a == out_g, "fold: -w 1 (every char) matches GNU")

    # Only spaces
    data = b"          \n"
    rc_a, out_a, _ = run_asm(["-s", "-w", "5"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-s", "-w", "5"], stdin_data=data)
    report_result(out_a == out_g, "fold: -s only spaces matches GNU")

    # Binary data
    data = bytes(range(256)) + b"\n"
    rc_a, out_a, _ = run_asm(["-b", "-w", "32"], stdin_data=data)
    rc_g, out_g, _ = run_gnu(["-b", "-w", "32"], stdin_data=data)
    report_result(out_a == out_g, "fold: -b binary data matches GNU")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (ffold) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
