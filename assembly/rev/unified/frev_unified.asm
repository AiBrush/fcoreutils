...
; UTF-8 aware character reversal.
...
; ─── utf8_reverse() ────────────────────────────────────────
; UTF-8 aware reversal: walks forward through line_buf[0..r15),
; places each character into out_buf starting from offset r12+r15
; and working backward. Updates r12 += r15.
; Clobbers: rax, rcx, rdx, rsi, rdi
utf8_reverse:
    lea     rdi, [rel out_buf]
    add     rdi, r12
    add     rdi, r15                    ; write_pos = end of output region
    xor     esi, esi                    ; read_pos = 0
...
.ur_loop:
    ...
    cmp     al, 0x80
    jb      .ur_1b                      ; 0x00-0x7F: ASCII
    cmp     al, 0xC0
    jb      .ur_1b                      ; 0x80-0xBF: orphaned continuation
    cmp     al, 0xE0
    jb      .ur_2b                      ; 0xC0-0xDF: 2-byte lead
    cmp     al, 0xF0
    jb      .ur_3b                      ; 0xE0-0xEF: 3-byte lead
    jmp     .ur_4b                      ; 0xF0-0xFF: 4-byte lead
...
.ur_2b:
    lea     rcx, [rsi + 2]
    cmp     rcx, r15
    ja      .ur_1b                      ; not enough bytes
    movzx   edx, byte [r14 + rsi + 1]
    and     edx, 0xC0
    cmp     edx, 0x80
    jne     .ur_1b                      ; invalid continuation
    sub     rdi, 2
    movzx   eax, word [r14 + rsi]
    mov     [rdi], ax
    add     rsi, 2
    jmp     .ur_loop
