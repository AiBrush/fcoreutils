#!/usr/bin/env python3
"""security_tests.py — Security & memory safety tests for frev (assembly rev)."""
...
BSS_SIZE = 65536
TOOL_NAME = "rev"
BIN = str(Path(__file__).resolve().parent.parent / "frev")
GNU = "/usr/bin/rev"
...
def test_proc_runtime():
    ...
    try:
        ...
        try:
            ...
            report_result(os.path.basename(exe) == "frev", "proc: /proc/PID/exe points to frev")
        except Exception as e:
...
def test_memory_safety():
    ...
    for desc, size in [
        ...
        # Single line of that size (rev reverses lines, so long lines matter most)
        data = b"A" * size + b"\n"
        rc, _, _ = run_asm([], stdin_data=data)
...
def test_concurrency():
    ...
    # rev | rev roundtrip pipe chain
    script = f'echo "hello world" | {BIN} | {BIN}'
    p = subprocess.run(["bash", "-c", script], capture_output=True, timeout=TIMEOUT, text=True)
    report_result(p.stdout.strip() == "hello world", "concurrency: pipe chain rev|rev roundtrip")
...
# =============================================================================
#                     13. TOOL-SPECIFIC: REV TESTS
# =============================================================================

def test_rev_specific():
    log("\n=== Rev-Specific Tests ===")

    # Basic reverse
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello\n")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello\n")
    report_result(out_a == out_g, "rev: basic reverse matches GNU")
    report_result(out_a == b"olleh\n", "rev: 'hello' -> 'olleh'")

    # Multiple lines
    data = b"abc\ndef\nghi\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: multiple lines matches GNU")

    # Empty input
    rc_a, out_a, _ = run_asm([], stdin_data=b"")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"")
    report_result(out_a == out_g, "rev: empty input matches GNU")

    # Empty lines preserved
    data = b"\n\nhello\n\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: empty lines preserved matches GNU")

    # Single character lines
    data = b"a\nb\nc\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: single char lines matches GNU")

    # Palindrome
    data = b"racecar\nmadam\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    report_result(out_a == b"racecar\nmadam\n", "rev: palindromes unchanged")

    # Spaces and tabs
    data = b"  hello  \n\tworld\t\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: spaces and tabs matches GNU")

    # Very long lines
    long_line = b"A" * 10000 + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=long_line)
    rc_g, out_g, _ = run_gnu([], stdin_data=long_line)
    report_result(out_a == out_g, "rev: very long line (10KB) matches GNU")

    # Roundtrip: rev | rev == original
    original = b"hello world\nfoo bar baz\n12345\n"
    rc1, mid, _ = run_asm([], stdin_data=original)
    rc2, final, _ = run_asm([], stdin_data=mid)
    report_result(final == original, "rev: roundtrip rev|rev == original")

    # Roundtrip with GNU
    rc1, mid_g, _ = run_gnu([], stdin_data=original)
    rc2, final_g, _ = run_gnu([], stdin_data=mid_g)
    report_result(final_g == original, "rev: GNU roundtrip rev|rev == original")

    # No trailing newline
    rc_a, out_a, _ = run_asm([], stdin_data=b"hello")
    rc_g, out_g, _ = run_gnu([], stdin_data=b"hello")
    report_result(out_a == out_g, "rev: no trailing newline matches GNU")

    # Special characters
    data = b"!@#$%^&*()\n[]{}<>|\\~`\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: special characters matches GNU")

    # Numbers
    data = b"1234567890\n0987654321\n"
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    report_result(out_a == b"0987654321\n1234567890\n", "rev: numbers reversed correctly")

    # Multibyte chars — must match GNU rev behavior
    data = "café\n".encode('utf-8')
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: multibyte chars matches GNU")

    # Many short lines
    data = b"".join(f"{c}\n".encode() for c in "abcdefghijklmnopqrstuvwxyz")
    rc_a, out_a, _ = run_asm([], stdin_data=data)
    rc_g, out_g, _ = run_gnu([], stdin_data=data)
    report_result(out_a == out_g, "rev: 26 single-char lines matches GNU")

    # CRLF
    crlf = b"hello\r\nworld\r\n"
    rc_a, out_a, _ = run_asm([], stdin_data=crlf)
    rc_g, out_g, _ = run_gnu([], stdin_data=crlf)
    report_result(out_a == out_g, "rev: CRLF input matches GNU")
    ...
    report_result(out_a == out_g, "rev: large input (10K lines) matches GNU")

    # Binary data per line
    binary_line = bytes(range(1, 128)) + b"\n"
    rc_a, out_a, _ = run_asm([], stdin_data=binary_line)
    rc_g, out_g, _ = run_gnu([], stdin_data=binary_line)
    report_result(out_a == out_g, "rev: binary data line matches GNU")

    # rev is from util-linux, uses -h/-V not --help/--version
    rc_a, out_a, _ = run_asm(["-h"])
    report_result(rc_a == 0 and len(out_a) > 0, "rev: -h (help) works")

    rc_a, out_a, _ = run_asm(["-V"])
    report_result(rc_a == 0 and len(out_a) > 0, "rev: -V (version) works")
...
def run_tests():
    log(f"=== Security Tests for {TOOL_NAME} (frev) ===")
    log(f"Binary: {BIN}")
    log(f"GNU:    {GNU}")
