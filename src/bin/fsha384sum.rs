...
/// Ultra-fast path for `fsha384sum <single_file>` — raw syscalls, zero allocation.
#[cfg(target_os = "linux")]
fn single_file_fast(path: &Path) -> ! {
    let mut out_buf = [0u8; 4096];
    match hash::hash_file_raw_to_buf(HashAlgorithm::Sha384, path, &mut out_buf) {
        Ok(hex_len) => {
            let name_bytes = {
...
mod tests {
    ...
    fn test_known_empty_hash() {
        ...
        assert!(output.status.success());
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b"));
    }

    #[test]
    fn test_nonexistent_file() {
        let output = cmd().arg("/nonexistent_xyz_sha384").output().unwrap();
        assert!(!output.status.success());
    }
