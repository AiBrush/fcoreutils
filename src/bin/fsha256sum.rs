...
/// Ultra-fast path for `fsha256sum <single_file>` — raw syscalls, zero allocation.
#[cfg(target_os = "linux")]
fn single_file_fast(path: &Path) -> ! {
    let mut out_buf = [0u8; 4096];
    match hash::hash_file_raw_to_buf(HashAlgorithm::Sha256, path, &mut out_buf) {
        Ok(hex_len) => {
            let name_bytes = {
...
fn main() {
    ...
    // Ultra-fast single-file detection: fsha256sum <single_file> (no flags)
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn test_nonexistent_file() {
        let output = cmd().arg("/nonexistent_xyz_sha256").output().unwrap();
        assert!(!output.status.success());
    }
