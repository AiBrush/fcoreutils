...
fn run(cli: &Cli, files: &[String], out: &mut impl Write) -> bool {
    ...
    for filename in files {
        let mut data: FileData = if filename == "-" {
            ...
            // With HUGEPAGE-first madvise ordering, mmap is faster than read()
            // for files >= 1MB: mmap with MADV_HUGEPAGE causes only ~5 minor faults
            // for 10MB (2MB pages) vs ~2560 faults for read() into Vec (4KB pages).
            // Small files (<1MB) still use read() via read_file_mmap's internal threshold.
