...
/// Strips newlines from input using SIMD gap-copy via memchr, pads if necessary,
/// then uses base64_simd for decode. Falls back to scalar for --ignore-garbage.
fn decode_base64_simd(data: &[u8], engine: &base64_simd::Base64) -> DecodeOutput {
    ...
    // Handle missing padding: GNU basenc accepts truncated padding but reports
    // "invalid input" for incomplete final groups (n >= 2 chars without padding).
