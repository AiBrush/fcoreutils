...
fn fold_one_line_column(line: &[u8], width: usize, break_at_spaces: bool, output: &mut Vec<u8>) {
    ...
    let mut has_tab_since_space = false;
    ...
    while i < line.len() {
            ...
            if col + tab_width > width && tab_width > 0 {
                ...
                if break_at_spaces {
                    if let Some(sp_after) = last_space_in {
                        ...
                        output.push(b'\n');
                        seg_start = sp_after;
                        // Recalculate col from sp_after to i
                        col = if has_tab_since_space {
                            recalc_column(&line[sp_after..i])
                        } else {
                            col - col_at_space
                        };
                        last_space_in = None;
                        has_tab_since_space = false;
                    } else {
        ...
        if break_at_spaces && byte == b' ' {
            ...
            col_at_space = col + cw;
            has_tab_since_space = false;
        } else if break_at_spaces && last_space_in.is_some() && byte == b'\t' {
            has_tab_since_space = true;
        }
